...
- **Known scope cuts, measured**: the strips do not **tile** — `stripFields` spaces grants
  ...
  **330 m of the market** (the near fringe is deliberately empty), and `FARM_SPEC` carries only
  ...
  Still absent from the reference's list and genuinely unbuilt: a village green as an object
  distinct from the market plaza, woodland as an **area**, a footpath street class, roof
  treatment by building function, detached outlying farmsteads, and the shore's sand band and
  shallow-water shelf.
