...
`Cartalith Gen1 v*.html`** (currently **v1.12**) — a zero-dependency HTML/JS/CSS application,
designed to open via `file://` (a local HTTP server is an accepted fallback for Workers/WASM
threads; `file://` must degrade gracefully, never break).

| File | Role |
|------|------|
| `Cartalith Gen1 v1.12.html` | **Current** unified tool (~20.7k lines, 4 script blocks — see architecture below) |
| `Cartalith Gen1 v0.57/v0.6/v0.61…v1.11.html` | Previous Gen1 versions (kept; never edit in place) |
