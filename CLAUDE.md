...
`Cartalith Gen1 v*.html`** (currently **v0.72**) — a zero-dependency HTML/JS/CSS application,
designed to open via `file://` (a local HTTP server is an accepted fallback for Workers/WASM
threads; `file://` must degrade gracefully, never break).

| File | Role |
|------|------|
| `Cartalith Gen1 v0.72.html` | **Current** unified tool (~15.6k lines, 3 script blocks — see architecture below) |
| `Cartalith Gen1 v0.57/v0.6/v0.61/v0.62/v0.63/v0.64/v0.65/v0.66/v0.67/v0.68/v0.69/v0.70/v0.71.html` | Previous Gen1 versions (kept; never edit in place) |
...
| `tests/` | Headless verification harness (`run.sh`, stubs, 897-assertion suite) + `tests/perf/` Playwright A/B + UI-smoke harnesses |
...
- **After any change to the engine (script block 1): run `tests/run.sh`.** A change is not done
  until it passes (897 assertions green).
...
1. **Generator engine + app shell** (~8.0k lines, `const VERSION='0.72'`). The full
...
```bash
tests/run.sh                        # newest Gen1 file: extract engine → node --check → 897-assertion suite
