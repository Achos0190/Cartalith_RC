...
| `Cartalith v2.73 DCC test.html` | **The DCC shell line's current head, not a mainline version.** The last two items v2.68's own closing line named as genuinely unbuilt: *a village green as an object distinct from the market plaza*, and *a footpath street class*. **NEITHER NEEDED NEW GEOMETRY.** **A GREEN IS A PLAZA WITH A DIFFERENT PURPOSE** — `buildPlaza` cuts a widened bay off the principal street, which is the geometry of a market place and of a village green alike, and what separates them is **market right, not size**: a chartered town's plaza is commercial, M-DEN-6's stall encroachment shapes the frontages around it, and a market CROSS stands in it as the legal marker of the right to trade; a village's green is common land (grazing, assembly, the pond) and carries no cross because there is no right to mark. So the plaza gains a `kind` and **nothing else changes — the same three `addStreet` calls, in the same order, at the same widths**. The threshold is **`buildCivic`'s own** (pop 1500, *"a civic hall appears once a place is a chartered town"*), never a second number for the same distinction (v2.63); measured as a crossover BOTH ways — 300/900/1400 green + pond + no cross, 1600/4000/12000 market + cross + no pond. **THE FOOTPATH HAD A FREE SOURCE THAT NEVER RUNS, AND ONLY MEASURING IT CAUGHT THAT.** `privatizeAlleys` models a through-alley taken into the adjoining plots: the edge must leave the STREET graph (no cart may route down it; blocks, routing, metrics and the wall trace must all keep seeing it gone) while the foot traffic survives — **that is what a snicket is** — so recording the killed line is four lines and free. It is also **unreachable on the profile the app generates**: the pass opens `if(!bias) return;` and `DEFAULT_RULES.street.deadEndBias` is **0**, only the medina family's documented 0.16 floor ever setting it — **0 paths across six populations on the default medieval profile**. Shipping that half alone would have been **v2.42's defect**. So `buildFootpaths` is the always-on source and **invents no geometry either**: a village's paths are the lines worn between the street and what people walk to daily, and the engine already places those — the church, the public wells, the pond — so any of them not already fronting a street has a path to the nearest one BY DEFINITION (nobody walks to the well through a hedge). **2 paths on a village, 4 at pop 12 000**, 7–17 m, bounded at both ends (`FOOTPATH_ON_STREET=6` — closer and it already fronts the street; `FOOTPATH_MAX=90` — beyond it the network owed a lane). **A PATH MAY NOT RUN THROUGH A HOUSE, AND THE FIRST CUT DID**: **11.4% of sampled path length fell inside a building footprint** at pop 12 000, because the nearest street point to a feature is not always reachable in a straight line once the plots are built out. Seven samples against the real footprints — the same call `buildParcels`' water test makes (v2.67), for the same reason — and a rejected feature simply gets no path, which is honest: there is no worn line where there is no way through. **0.0% at every population after.** **ORDER IS LOAD-BEARING**: after `privatizeAlleys` so a path can never end on a street about to be enclosed, and after `buildBuildings`, so — like the alley capture — it cannot move a block, a parcel or a building; **`hashModel` covers graph/blocks/parcels/buildings and NOT `plaza`, `details` or `paths`**, so the 882 UM goldens cannot move BY CONSTRUCTION, asserted directly against v2.72 over six populations rather than as "the goldens still pass" (v2.66). **ONE helper each, three renderers** (`_umDrawGreen`/`_umDrawPaths`/`_umDrawPond` from `_umDrawLayout`, `_umDrawLayoutPreview`, `_cvDrawCity` — v2.68's rule), each gating on **its own** size reaching half a pixel and never a shared zoom tier: **v2.50's "a floor that INFLATES is not a bound" applies to a renderer too**, since a 1.4 m path floored to 0.5 px reads as wide as the street beside it. The green is GROUND and has no gate; the pond and the path are the size of a person and are disclosed only at their own scale. **TWO PROBE ASSERTIONS OF MY OWN WERE WRONG, AND BOTH CORRECTIONS ARE THE USEFUL PART**: the first drove `_umDrawLayout` through an arbitrary `toScreen` multiplier and read the correct silence as a broken renderer — **that function derives its own `mScale` from `lodSpanKm()`, the LIVE camera span, not from the transform it is handed**; the second then asserted a span floor where none is — **`lodSpanKm()` returns a sub-metre span quite happily and the `Math.max(1,…)` floor is in the CONSUMER**, which is where a real ceiling comes from and is now pinned as one: `_umDrawLayout` **saturates at `GW/1000` px/m**, so on a 256-cell grid a 1.4 m path tops out at 0.36 px and is correctly never drawn on the main map, while every resolution the app ships at (512/1024/2048) clears its gate. `hash_gen1.js` vs v2.72 **ALL IDENTICAL**. **Known scope cuts**: the green is chosen by population alone, so a small chartered borough still gets a market place and a large unchartered village still gets a green — a charter is not a thing a settlement can hold; the pond is one ellipse with no outflow and no relation to the real water table; a path is a straight segment rather than a worn curve, and none runs between two features (feature-to-street only); and only churches, wells and the pond are destinations — not the mill, the gate or the fields. Verify with `tests/perf/probe_greenpath.js` (21 assertions). |
| `Cartalith v2.72 DCC test.html` | Previous DCC-line file. Owner, on a 40 000 km world (seed 61684), with a screenshot: *"The hell is going on here???"* — then, once the lines were gone, *"Still the density of rivers make it look like a big block. It's ugly."* **TWO defects, both in the RASTER river path, both invisible at the app default, and both landing on the wrong one of two renderers.** **(1) THE LINES ARE RIVERS.** `net.recv` wraps in X in world mode, so consecutive points of one stem sit at x≈1023.5 and x≈0.5; `riverFieldTile` stamps a band along every segment, so one wrapped step paints a river clean across the map. `splitRiverPolylines` exists for exactly this — v1.29 built it for the stroked overlay and the GeoJSON export, v2.37 added the carve as the third site — and `riverRenderPolys`, the geometry the raster path draws from, was the fourth and never got it. **The raster path is the one that draws at the default** (`riverWays:false`, v2.29), so the renderer that splits is the one nobody was looking at. **v2.58 unwrapped the LENGTH in the very same loop** (`dx -= Math.round(dx/GW)*GW`) while still handing the drawer the wrapped points — which is why this read as fixed for fourteen versions: **fixing a measurement of a quantity is not fixing the thing the quantity describes.** Every river probe runs region mode, which cannot wrap, and the hash battery never sets `state.world` — v2.37 recorded that blind spot and it caught the carve, not the render. Measured: wrapped segments **44 -> 0**, worst step **1023 cells of 1024 -> 1**, rows with their horizontal detail wiped out **52 -> 0**. **(2) A DETECTION EASE IS NOT A DISPLAY THRESHOLD — v2.57's finding at its third consumer.** `riverCoarseEase` answers v1.101's question, whether a stream EXISTS on a coarse map; v2.57 split `carveFlowThresh` out of it because that says nothing about whether the grid can hold the stream's VALLEY, and it says nothing about whether the MAP can legibly carry it either. The render spent the full **16x** ease on what it draws: **13.02% of the map painted as channel at 40 000 km against an 800 km region's 4.27%**, a 3x denser network drawn at a uniform floored width (v2.60) in an opaque lake colour (v2.61) — three individually-correct things compounding into a solid block. The median stem is the **same length in cells at both extents** (5.7 vs 6.4), so this is density, not size. **v2.58's screen-px gate was measured FIRST and REJECTED**: at `_z=1` it culls **88% at BOTH extents** (761 -> 93 on an ordinary 800 km map), fixing one world by gutting every other — and inspecting it showed why the gap existed at all, since **`RIVER_MIN_SCREEN_PX` has exactly ONE consumer, inside `drawRiverWays`, the overlay that is off by default**, so v2.58's whole scale-selection feature was built onto the renderer nobody sees. `RIVER_RENDER_AREA_K x riverCoarseEase` is the bar instead, on each stem's own upstream drainage in CHANNEL CELLS (`buildMainStems`' Kahn accumulation, never `flowField` — v2.41/v2.58: two different trees). **At and below 800 km the ease is 1, the bar is 2, and every stem of two or more points clears it trivially — 0 of 833 rejected, so the default is unchanged BY CONSTRUCTION**; at 40 000 km **7 220 of 7 827** are rejected and the channel share falls **13.02% -> 3.58%**, just under the region map's own 4.19%, which is the target rather than a tuned number. **A probe assertion of mine was CORRECTED, not loosened**: it compared stems drawn against stems available, read 761 of 833 at 800 km and looked like the new bar culling 72 — that gap is v2.61's pre-existing "shorter than it is drawn wide" cull, measured at exactly 761 before this change. **Comparing across two culls attributes one's work to the other.** `hash_gen1.js` vs v2.71 **ALL IDENTICAL** and region-mode rgba is bit-identical on both halves, so 800 km maps do not move. **Known scope cuts**: the seam split can leave a short run on one side, and v2.61's length cull is applied to the whole stem BEFORE the cut rather than to each run after it; `RIVER_RENDER_AREA_K=2` is calibrated against the region map's density, not against a hydrological figure; a stale baked atlas still carries the old lines until re-baked (tiles have taken this geometry since v2.40). Verify with `tests/perf/probe_riverworld.js` (13 assertions). |
| `Cartalith v2.71 DCC test.html` | Previous DCC-line file. Owner: *"do woodland areas next and on part of settlements, your example rendered in water (either lake or sea it should avoid and always have suitable land beneath it"*, then — after the first fix passed every synthetic fixture — *"within the actual cartalith generation it seems to miss. Do we need an extra technical layer to help guide settlement generation or can we use an existing layer?"* **THE SECOND REPORT IS THE ONE THAT MATTERED, AND THE ANSWER WAS AN EXISTING LAYER.** On the REAL map-water path `buildSite` sets **`waterPoly=[]` deliberately** — its own comment says the map already draws the sea beneath the town, so the town must not paint a second one. Correct for FILLING, and it leaves every renderer with **no water geometry at all** on exactly the towns the app generates: a clip keyed on `waterPoly` passes every fixture and does nothing live (**7 of 39 real towns carry an empty one**). No new layer was needed — `_umWaterCtx` has always built a 22 m mask of the real sea, lakes and river band, and **`isWater` reads it already**; it simply never reached the model, because `cityGen`'s site record is function-free and the mask was not among the fields copied. `landRuns` is that mask run-length encoded, carried across (**38 of 39**), its four corners pushed through `gxy` so the clip inherits the layout's rotation and the camera — a screen-space rect is axis-aligned and wrong the moment the town is turned. **A 22 m stair-step is finer than the coarse grid's own ±half-cell uncertainty about where the coast is** (v2.60), so marching squares would be eighty lines to make a boundary smoother than the data under it. **ORDER IS LOAD-BEARING — the mask beats the polygon**, and preferring the polygon was a real defect in my own first cut: a real town carries BOTH (54–113 runs alongside `waterPoly`s of 4/6/10/14 points built from a river centreline), so the polygon clips the town against a sliver of one channel and leaves everything else standing on open sea. The mask is what `isWater` itself reads; the polygon is the synthetic path's fallback. **A CLIP, NOT A GEOMETRY FIX, and the measurement is why**: ZERO blocks, parcels, buildings or fringe parcels have a sample inside the water and ZERO street CENTRELINES do either — what crosses is the street's **WIDTH**, stroked ~8.4 m with its casing, and `removeWaterCrossings` samples centrelines only. Insetting streets leaves blocks, walls, buildings and every future feature to be fixed one at a time; the clip states the requirement once, this version's own woodland included. Released for the bridges and the ford, which are MEANT to span water. **The fringe needed its OWN span** — it must stay drawn BEFORE the water fill (v2.68), but where there is no water fill "drawn under the water" stops being a backstop, and two real towns kept 24 and 4 pixels that were all fringe. **TWO CHEAPER MEASUREMENTS BOTH LIED, which is the most reusable thing here.** Overdrawn water as a SHARE of the water body reads **0.14%** and looks like antialiasing, because the denominator is a 211 000-px sea while every wrong pixel is in the one place the eye goes; matching pixels against a built palette misses it **entirely**, because a street edge over water is antialiased and matches no entry. The honest test renders the town, renders it again with the settlement layer stripped, and diffs INSIDE the water. **And one of my own visual calls was simply wrong** — the magenta waterline appeared to run through the shore blocks and was the 1.5 px stroke straddling a street that stops exactly at it; the coast fixture had **zero** encroachment all along and the real one was `riverthrough`, where `removeWaterCrossings` exempts primaries as presumed bridges and that town had **0 bridges and no ford**. Real world, before → after: **36→0, 61→0, 24→0, 1→0, 4→4, 0→0**, control worst **61 px against 4**. **WOODLAND IS AN AREA, because a point scatter cannot make a mass** — the engine has scattered `kind:'tree'` points since v0.95 and no density of stipple produces an edge, so `buildWoods` emits a polygon and puts a handful of trees INSIDE it as texture on the mass rather than as the mass: **9–10 woods on every site kind and size**, none on water, none on a field. **The plough wins and `farm` is an input for that reason** (assarting clears woodland FOR fields, so the arable is the later, winning layer), and there is **NO `crossesStreet` guard** — a furlong with a road through it is not a furlong, a wood with a road through it is an ordinary wood. **THE FRAME WAS WRONG BEFORE IT WAS MEASURED, TWICE**: keyed on `maxRF` the band ran to `maxRF*2.8` (~730 m) while the arable owns everything out to ~650 m, so every surviving wood landed in the sliver between — **672, 687, 688, 694, 724, 745, 750 m**, a ring one wood thick. Loosening the farm clearance and dropping the street guard each moved the count by **zero**; only reframing on the site BOX did. **Guessing at a constraint twice and measuring once is the wrong order, and the measurement was cheap.** **A probe assertion was CORRECTED, not loosened** — `probe_fringe`'s hedgerow gate counted every fill as its denominator and woods stroke in their own colour; scoping the denominator to field/pasture passes on **v2.70 as well**, which is what makes it a correction. `hash_gen1.js` vs v2.70 **ALL IDENTICAL**. **Known scope cuts**: the woods band wherever the field fans leave room, which on a road-fanned town is one sector — correct by the model and it reads as a belt rather than scattered copses; no clearings or ponds inside a wood; woods have no relationship to the real biome, so a desert town still grows them; the clip's residual is one antialiased pixel along the waterline. Verify with `tests/perf/probe_woodland.js` (10 assertions). |
| `Cartalith v2.70 DCC test.html` | Previous DCC-line file. Owner, having pointed at a reference village map: *"And for the coloration and style can we add it as a preset in the styles section under carto."* **THE AUDIT DECIDED THE SIZE OF THIS, and it is ONE KEY** (v2.58's rule): `landColorCore` is the one function that colours land and it has **four call sites** — the main map's per-pixel loop, the LOD tile renderer and the flat bake — and it already ends in a chain of per-pixel NPR steps each gated on its own `state.viz` key (`cel`, `crosshatch`, `stipple`, `sepia`, `risograph`, `pointillism`); `seaColorCore` is its twin for water. So the feature is **one more step in each chain, one key, one `STYLE_PRESETS` row and one button**, and every surface that draws a map gets it free — which is what the LOD-tile and bake assertions prove and what a canvas-level tint would fail. **The ramp is keyed on LUMA (land) and DEPTH (sea), never on biome** — the same reasoning `risograph` beside it uses: the lit colour has already absorbed slope, aspect, material, hillshade and every biome distinction, so quantising IT keeps all of that and removes only the continuous gradient, which is the whole difference between a relief map and a drawn one. **The stops are not invented** — they are the colours this file already paints the settlement layer with (v2.68's own ground/field/pasture/road/water tones), so terrain and town finally read as one drawing. **`shadows:0` is load-bearing, not taste**: the hillshade is applied BEFORE the ramp, so leaving it on puts a gradient straight back into the thing whose point is not having one. The sea step is a **REPLACEMENT at the very end**, not another mix into `wc`, because the seabed grain, the bathymetric hillshade and the smooth depth ramp are all already folded in by then. Measured: pixels move, **Default returns the render BYTE-IDENTICAL** (measured inside one build against its own baseline — which is why `village` had to join `STYLE_MANAGED_NUM`), the LOD tile and flat bake both pick it up and both return, water banding **62 -> 17**. **TWO OF THE PROBE'S OWN ASSERTIONS WERE WRONG FIRST**: reading the canvas straight after `_applyStylePreset` returns the PREVIOUS frame (it ends in the deferred `render()`), so it reported *"the preset changes nothing"* and caught the change one assertion later; and counting EXACT distinct sea colours read **1322 -> 1322** against a screenshot that plainly showed the sea flatten, because an 0.88 blend leaves 12% of the original carrying the seabed grain — 16-level buckets measure BANDING, which is what the claim means. **Reaching the control takes two navigations and that is correct**: the row sits in a category accordion inside the Cartography panel, so the probe opens the domain from the real rail button and the accordion from its own `<summary>` (found by walking UP from the button, not by a hand-written selector) before asserting the click LANDS (v2.46). **Known scope cuts**: the civ layer's ways, territory fill and labels keep their own colours; lakes follow the sea through `seaColorCore` but the river overlay's stroke does not; no user-editable palette. Verify with `tests/perf/probe_villagestyle.js` (14 assertions). |
| `Cartalith v2.69 DCC test.html` | Previous DCC-line file. Owner: *"when I zoom in and the LOD renders deeper it often happens that the coastline, or rivers for that matter, don't align properly with the settlement."* **REPRODUCED AND DECOMPOSED BEFORE ANYTHING CHANGED, and the decomposition picked the fix**: share of a refined tile's pixels drawn as the OPPOSITE of what the settlement's own surface says — **z2 0.40%, z4 0.86%, z6 5.73%, z8 8.56%**, monotone in depth, which is the report in one line. **THE DOMINANT CAUSE WAS THE FILTER, NOT THE NOISE**: `burnChannels` and `featureDetailPass` are the obvious suspects and **neither runs at the app defaults**, while v2.52's crater registry runs and makes **exactly zero** difference — what was left is that `_umWaterCtx` decided the town's coastline with a hand-written BILINEAR sample while **v2.47** moved the tile's own reconstruction to `sampleC1` (**17x the disagreement**: 2.89% against 0.17% with the filters matched). The file's most-repeated defect landing between an adapter and a renderer. **THE SECOND CAUSE IS A ONE-SIDED TAPER**: `amplifyRegion` fades detail out going DOWN from the shelf and does nothing going UP, so a land pixel a hair above sea took the full band and could be pushed under — **2.31% land->sea against 0.30% the other way, a 7.6:1 asymmetry, further at every level**. **v2.40's rule is that refinement adds RESOLUTION and does not invent**, and the land/sea boundary is a decision placement, `_umWaterCtx`, `_civLakeFlooded` and the road network are all built against. **The clamp is on the DELTA, not the result** — clamping `v` to `sea` pins a coastal band to one value and makes a flat shelf (**v2.50 in a renderer**), so the excursion TOWARD sea level is capped at **half the remaining headroom**: no constant, never reaches the far side, and confined by construction (a pixel whose detail is under half its own headroom is bit-identical). **THE FIRST CUT GUARDED THE WRONG FUNCTION**, and only the control said so — the band is added in **two** places and a clamp in `addZoomDetail` alone left 3.33%, caught as *"control 1158 drowned, fixed 1158"*, an identical number on both sides that reading the diff would never have produced. After: **z2 0.35%, z4 0.10%, z6 0.28%, z8 0.13%**, worst level **8.56% -> 0.35%**, **0 of 41 984** pixels drowned against 1 158; v2.47's own `probe_lodsurface.js` still **12/12** with seam delta **exactly 0**. A deliberate bounded re-baseline of **LOD tiles/baked atlas chunks within one detail-amplitude of sea level** (a stale atlas wants a re-bake) and **generated town layouts** (the water mask is in `_umCacheKey`) — never `field`. **DISCLOSED, MEASURED, NOT FIXED: the river half is a different defect and it is a WIDTH mismatch, not an offset** — the centreline is shared by construction (`_civRiverPolylines()`), but `_umWaterCtx` derives width as `10+order*7` capped at 46 m, its own formula, while **v2.49** gave the renderer a real hydraulic half-width in `_riverNet.halfw[]` and recorded that both river renderers "were built for this and both receiving a constant". **CORRECTED AND CLOSED**: those first figures (9.1x/18.8x/24.7x) compared the town to the RAW `halfw`, which is NOT what the map draws — the drawn width also carries v2.60's `RIVER_MIN_HALF_CELLS=0.8`, a floor in GRID CELLS, so at the app default **every order draws at 2500 m against a 1700 m town box**, the order distinction collapses entirely, and the real ratio is **~104x, not 24.7x** (at 200 km the floor binds on order 1 only; at 50 km never). **Decided by the owner: LEAVE IT, because both numbers are honest** — v2.60's floor is a RESOLUTION statement (the centreline is known to +/-half a cell, so a narrower channel claims precision the data lacks) and at 800 km one cell IS 1.5 km, so the map cannot locate a 30 m river; the town's 24 m channel is the physical truth and the band is the positional truth. Every way of forcing agreement costs more than the disagreement (narrowing re-baselines every tile and risks v2.60's 0 -> 111 broken stems; widening drowns the village; suppressing inside the town box helps only where a town is drawn). A known limit of a 1.5 km cell, not an open defect. Verify with `tests/perf/probe_lodalign.js` (11 assertions). |
| `Cartalith v2.68 DCC test.html` | Previous DCC-line file. **A WHOLE FEATURE WAS GENERATED AND RENDERED NOWHERE**: `buildFarmland` has pushed `field`/`pasture` polygons into `model.details` since v0.95, neither map renderer reads `model.details` at all, and the City Viewer's own detail pass (v1.18) branches on well/cross/crane/bollard/spoilheap/tree/dryingrack/logboom/fence with **no branch for either kind** — so every field fell through every `else if` and drew nothing, in silence, for the life of the file (**v2.65's defect one layer down**: a lookup that silently skips an unknown kind hides a whole feature; v1.17's own scope-cut line said `model.details` was never drawn and was only ever half-closed). Measured on the shipped generator: **62-86 field and 17-22 pasture polygons per pop-440 village**, 98 on the coast fixture. This adds NO generation and changes NO value — `hash_gen1.js` vs v2.67 **ALL IDENTICAL** — and **the control half is what makes that evidence**: the probe opens v2.67, confirms it generates *the identical 98 parcels*, and confirms it draws **0 in all three renderers** (v2.39). **THE SELIONS ARE THE HATCHING, which is why no hatch pass is written**: the reference shows furrow lines whose direction changes per parcel and the obvious build is a clipped hatch plus a stored bearing, but `stripFields` already cuts each grant at **5.8 x 91.5 m, every poly a quad, aspect 15.8:1** — a *selion*, one plough-run, and a furlong is a bundle of parallel selions. Measured over 98 parcels: **10 distinct bearings, largest bundle 52 sharing one**. Storing a plough bearing would be a second way to say what the polygon already says. **The hedgerow gate is a CROSSOVER keyed on the parcel's OWN shortest edge**, not a global zoom threshold — a 5.8 m strip drawn 0.6 px wide with a 0.5 px dark outline reads as the *outline*, so the fringe would come out one dark wash (**v2.50's "a floor that INFLATES is not a bound", in a renderer**); asserted both ways, 98 fills / **0** hedgerows at 0.08 px/m and 98 / **98** at 1.0. **The fringe is GROUND so it goes down FIRST, under the water** (it spans **1726 x 401 m against a 187 x 332 m built mass**), and the probe asserts the ORDER, not that both were drawn. **ONE helper, three renderers** (`_umDrawFringe`/`_umDrawFringeProps` from `_umDrawLayout`, `_umDrawLayoutPreview` and `_cvDrawCity`) — three call sites is exactly where this file's drift lives. Trees and yard fences **MOVED** out of the City Viewer's `CV_LOD_MAX` glyph pass into that helper rather than being added twice; each prop gates on **its own** size reaching half a pixel (**28 trees -> 0 marks at 0.02 px/m, 28 at 1.0**). **Disclosed, measured, NOT fixed**: the strips do not TILE — `stripFields` spaces grants **28-40 m apart at 4-7 m wide**, so the fringe reads as separated slivers where the reference shows a contiguous fabric; closing that is a generator re-baseline with its own measurement and would spend the very guarantee that makes this version checkable. It also skips anything within **330 m of the market**, and `FARM_SPEC` carries only `medieval` (strip) and `venus` (ring), so every other profile generates no farmland at all. `tests/run.sh` **1280/0**, `run_um.sh` **882/0**. Verify with `tests/perf/probe_fringe.js` (13 assertions). |
| `Cartalith v2.67 DCC test.html` | Previous DCC-line file. **TWO FUNCTIONS WERE ANSWERING ONE QUESTION, and one could only say "near" or "far"**: `buildParcels` asked "core or fringe?" with a hardcoded `dM<160` and spent the answer on one number, while `assignDistricts` asked the same question 130 lines later with the plaza, the wall ring, the river, the quay and the market radius and produced **seven wards** — so a harbour, a suburb, an agrarian fringe and a riverside craft quarter all platted identically, with the harbour's own source comment citing §1.1 #22 (*"deepest plots at quay; plot frontage narrowest of any family"*) as its reason for existing. `makeWardAt` is that chain extracted verbatim, built ONCE in `cityGen`. **Measured before anything was built, and it changed the design**: ward-driven DEPTH would have been v2.42's defect — **67.6% of parcels come out BELOW `depthTarget`'s own 14 m floor** (the block waist binds, not the draw) and tripling `plotDepthVariance` 0.22 -> 0.60 moves median depth **11.09 -> 11.07 m**. So what ships is the **SUBDIVISION**, which is what the ask names and what is live: `splits` was driven by STREET AGE alone and the halving chance was a flat 0.4, while M-PAR-1's own row says mature widths come from *"grant + subdivision history (p_split per epoch ~0.1-0.2)"* and what selects a plot for halving is the VALUE of its frontage. `WARD_GRAIN` is **one row per documented quarter**, and **no row is deeper than 30 m** — the deepest base the old proxy could already choose — because depth buys nothing and a deep narrow plot is what spans a channel. Effect, as a per-ward sign against each ward's OWN baseline: market **−1.36 m**, burgher −0.68, harbour −0.53, artisan +0.03, suburb **+1.57**, agrarian **+1.60**; core-to-fringe gap **0.88 -> 3.85 m**; market 1 528 -> 1 721 parcels while agrarian 419 -> 378. **A first cut of that assertion was wrong and fixing it is the more useful statement** — mean frontage per ward is NOT a function of pressure alone, because the BLOCKS differ per ward, so the claim is per-ward against itself. `wardGrain` (new `DEFAULT_RULES.parcels` field, default 1) makes it controllable and **0 is BIT-IDENTICAL by construction** (asserted over 30 towns, 0 divergent); the default re-baseline is **441 of 20 316 parcels (2.17%)**. **THE CORNERS ARE NOT THE FOOTPRINT EITHER** — the goldens caught one **craftriver parcel 41.3 m deep on a 5.8 m frontage with every corner dry and its middle in the river**; the hole is PRE-EXISTING (corner-only since v0.95) and was unreachable while every block platted at one grain, proven by measuring v2.66 at `plotDepthVariance` 0.60 where depth reaches the 46 m clamp: **zero wet parcels**. `par.ward` is written on every parcel (the PLOT SERIES' ward, not the parcel's re-tagged district). **`run_um.sh`'s total is not a constant** (852 -> 882): its Venus check is per-BUILDING. `hash_gen1.js` vs v2.66 **ALL IDENTICAL**. Verify with `tests/perf/probe_wardgrain.js` (23 assertions). |
| `Cartalith v2.66 DCC test.html` | Previous DCC-line file. Owner: *"Design a proper menu to generate previews for settlement types based on parameters. See the proof of concept settlement generation for an example."* **The audit moved most of the work off the table before any was done** (v2.58's rule): block 4 has exported **`DEFAULT_RULES` — 22 named generation parameters** (street / parcels / settlement) plus `resolveRules`/`applyWildness`/`applyPlotChaos` since v0.95, and `cityGen` reads `opts.rules` on its first lines — and **`_umPlaceContext` has NEVER set it**, so every town this app has ever drawn came out at `DEFAULT_RULES` with no control anywhere (**v2.63's shape one layer down**). The PREVIEW half existed too: `_umDrawLayoutPreview` takes a bare model, and `cityGen` runs against a SYNTHETIC site — the path `run_um.sh`'s 852 goldens exercise. So this is a context builder, a pane, and **one wire**. A **fourth pane in v2.63's own screen**, not a fourth window: Global → **Types** → Factions → Settlements is one scope ladder, and the shell, the `data-pm` bar, the header button and the ✕/Escape contract already existed (v1.57/v2.46 by construction). **What is SAVED per type is the rules object and nothing else** — site, culture, specialisation, walls, population, age and seed steer the preview only, because each already has a per-settlement home, and a second per-type copy is v1.57's defect. `state.civTypeRules` starts `{}` and `civTypeRulesFor` returns **null rather than an empty object**, so `rules` is ABSENT and bit-identity is **structural**; the probe asserts that **rendering the pane stores nothing**, or opening the screen to look would author every type it touched. **Assert the EFFECT as an ORDERING** (v2.42/v2.64): Medina's `deadEndBias` 0.40 leaves more cul-de-sacs than Planned Grid's 0, and its `subdivisionCap` 4 cuts more parcels — a no-op wiring satisfies neither. **The rules reach the MAP**, asserted on a real placed town by `UME.hashModel` before/after, and **`_umCacheKey` had to carry them** or a stale layout survives for ever and silently (v1.28). Every list is built from its own table and the probe asserts each COUNT against that table (v2.65's rule), which also pins that every `UM_RULE_SPECS` key names a real `DEFAULT_RULES` field and every default sits inside its own slider range. **And exposing the table reached a NON-TERMINATING REGION of the engine's own documented range**: `buildParcels` re-draws a frontage grant until one fits the remaining edge, with no bound, so when the remainder sits just above the 4.5 m floor the only escape is the lognormal's far lower tail — measured against a 4.6 m remainder, **0.40 escapes in ~68 draws, the 0.22 DEFAULT in ~28 571, 0.18 in ~2 000 000, and 0.12 (the PoC's own 'Planned Grid' profile) effectively never**. Latent for the life of the file because nothing ever set `opts.rules`. **Found by BISECTION** — all nine street parameters measured ~450 ms alone at pop 900 and the parcels group alone did not return. The fix is a BOUND, not a new formula (`PARCEL_GRANT_MAX_SPIN`) — **and the obvious claim for it, that it cannot change a terminating run, is FALSE, which measuring it is what caught.** `run_um.sh` is 852/852 so the goldens never reach the bound, but an ordinary default-variance town does (true max **172 644** spins against 4096), so this is a **deliberate, bounded re-baseline of generated town layouts**, sized by measurement: raising the bound to 2^22 and regenerating, **10 of 12 towns differ, worst 10 parcels (1.08%), 46 of 8 939 overall (0.51%)**. A probe asserting only “the goldens still pass” would have reported a byte-identity that does not hold. All six presets × eight populations to 20 000 now return in **440–930 ms**. `hash_gen1.js` vs v2.65 **ALL IDENTICAL** (urban layouts are opt-in). Verify with `tests/perf/probe_typepreview.js` (41 assertions). |
| `Cartalith v2.65 DCC test.html` | Previous DCC-line file. `docs/05` §7.3's closing bullet asks for one thing — the status gradient is *already* implicit (density, centrality), so **make it explicit in districts**. `par.status` is that number, built only from what `assignDistricts` already had: proximity to the market (M-NET-10's integration proxy), intramural-or-not (§3.1's suburbs row — a faubourg is cheaper ground), and how far DOWNWIND the parcel sits on v2.64's bearing. No new pass, no constant of its own, written on EVERY parcel. Measured, it reads as a gradient rather than a label: **0.662 near the market against 0.218 at the edge**, and — the half only v2.64 made possible — outer ground **downwind 0.143 against upwind 0.403**. §3.1 summarises the poor quarter as "to the edge AND downwind" and both terms are in the one number, so the selection is a single sort rather than a second rule. Patrician is carved out of the BURGHER ring, never the commercial core (§3.1 row 2: the prime frontages *adjoining* the market) — 50 parcels at mean **0.573** against 44 slum at **0.050**. **Also closes a v2.64 gap of my own**: there are TWO palettes — `_UM_ECON_TINT` colours the BUILDINGS, `_UM_DISTRICT_FILL` the PARCEL at the City Viewer's `CV_LOD_CITY` tier, where `if(!fill) continue` silently skips an unknown district — and v2.64 added its four to the first only, so they drew with no quarter fill; **its own probe asserted only the one palette**. Strengthening that assertion to cover every district actually OBSERVED rather than a hand-list then found a **PRE-EXISTING** hole: `buildFaithSites` has always tagged precinct parcels `church` and this palette never held one (confirmed absent in v2.63), so a cathedral close drew with no fill. **The hand-list was the defect; the derived assertion closed it.** `hash_gen1.js` vs v2.64 **ALL IDENTICAL**. Verify with `tests/perf/probe_industry.js` (29 assertions). |
| `Cartalith v2.64 DCC test.html` | Previous DCC-line file. `docs/05` §7.1 names two cheap site-model additions as what unblocks industry siting — a prevailing-wind vector and an along-water gradient — and **neither existed**. Shipping them alone would leave two fields nothing reads (v2.14), so they ship with their first consumer, the §4.7 table (**M-IND**). **Neither is invented per seed**: the wind is `currentWindField()` and the downstream direction is **v2.62's `_civRiverFlowField()` receiver tree**, returned as BEARINGS in the layout frame — v1.17's `_umOreBearing` convention, so `assignDistricts` consumes them the way it already consumes the ore bearing. **Two defects in my own first cut, both found by measurement.** (1) The nearest-channel search selected on `km2>0`, which `_civRiverFlowField` fills for EVERY cell from `flowField` while filling `fx`/`fy` only on channel cells — so it landed on the town's own dry cell and found no vector: **1 of 14 towns got a bearing, 14 of 14 after**. (2) The obvious reading of §4.2 — riverside AND `along>0` through the market — produced **ZERO tan yards across 14 towns** while the mirrored test produced mill races, because **one town carried 62 riverside parcels and all 62 sat upstream of its market**: a river clips the box on one side and the market is not in the middle of the frontage, so *"downstream of the market" is unsatisfiable for most towns*. Both trades are ranked along the ONE vector instead — tan yards at the downstream END of the town's own frontage, mill races at the upstream end — **the same insight as v2.62: the direction is real, the origin is not**. The probe therefore asserts the ORDERING (every tan yard downstream of every mill race in the same town), which is strictly harder than any sign test. The wind half needed no correction: **24 kiln yards, 0 upwind**; 20 inn yards at the gates. These are properties of the SITE, not of the town's trade, so the pass sits outside the specialisation chain and never steals a parcel that chain claimed — **one industry per parcel, first claim wins**. Absent `opts.economy` it is inert, so the 852 UME goldens are untouched by construction (v0.98). All four districts carry tints and provenance, asserted — a district with no tint renders default brown, the v1.80/v2.15 invisible-feature defect. Verify with `tests/perf/probe_industry.js` (23 assertions). |
| `Cartalith v2.63 DCC test.html` | Previous DCC-line file. Owner: *"As with the asset manager id like a window/screen were I can modify settlement parameters, global per faction but also per individual settlement."* **Two of the three scopes already existed and the third had no control anywhere** — per-settlement is `_civPopulatePlaceEditor` (12 fields, reached from the map popup, the sidebar list and the virtual table), per-faction is `_civPopulateFactionEditor` (5 fields, v1.57's pop-up), and **global was ten constants, each declared once, each hardcoded, with no UI at all**. Both existing builders already take a `host`, so this adds a second HOST and **not a second editor** (v1.57). **Three of the ten are deliberately NOT exposed**: `FARMERS_PER_URBANITE` is already per-faction through v1.54's Ag. technology rungs, `FOOD_BASE_SURPLUS_RATIO` is the constant `foodSurplusRatio()` pins itself to, and `SETTLE_COAST_SWAP_TOLERANCE` is an internal tolerance. **ONE list, THREE consumers** — `CIV_PARAM_DEFS` holds each number once and the default, the live `civParam()` read and the Global pane's rows all come off it; the four block-2 constants that owned their own literal now read `civParamDef` (a second copy is how v1.72 BUG-A / v2.45 / v2.54 each drifted). **`state.civParams` starts EMPTY**, so bit-identity is structural rather than checked and a pre-v2.63 save reloads as the world it was (the v2.17 `passes` convention). **A knob that stores a number nothing reads is the v2.42 defect**, so the probe measures the EFFECT: threshold 0.30 seeds **94** sites against the default's **67**, 0.60 seeds **18**, and clearing restores the default exactly. **The surplus ceiling had to move BOTH ag-tech branches** or it means two different things — R=9 **0.350 -> 0.556**, R=6 **0.450 -> 0.571**, R=1 **unchanged at 0.750** (correctly inert; the cap is not the binding constraint there, and the probe asserts that too). A change applies on the **next** Auto-populate — nothing regenerates behind a slider drag (v2.38's 11.2 s pass), and "Re-populate now" clicks the real `#civAutoPopulateBtn` so that handler stays in one place. Reached from the **HEADER, not the cog** (v2.46), asserted by a real click LANDING; once open it covers the header, so ✕/Escape are the way out (v1.57's shell contract) and what is pinned instead is that `aria-pressed` can never lie about being open. `civParams` joins `GEN_PARAM_BLOCKS`. `hash_gen1.js` vs v2.62 **ALL IDENTICAL**. Verify with `tests/perf/probe_settleparams.js` (30 assertions). |
| `Cartalith v2.62 DCC test.html` | Previous DCC-line file. Owner: *"these rivers should be navigateable as a route (but with a lower friction/cost) and a course or direction the water flows (this also informs the travel planner)"*. **A per-cell cost cannot tell walking ALONG a river from cutting ACROSS it**, so the ford was charged on every river CELL (following a channel priced as fording it once per cell) and the navigable discount was multiplied into every river cell (a road crossing a trunk river square-on collected 35% off the cell it was bridging). Measured before: an order-3 river cell — the only kind a hull can use — cost **3.46x** plain ground, its 1.894 h of bridge dwarfing 0.587 h of travel. **The most navigable river on the map was the most expensive ground on it.** This is **v2.33's own finding about slope** (*"it is the only place a direction exists"*), which that version moved to `edgeCost` and did not carry to the river terms ten lines below. Both move to the edge, scaled by how squarely the step cuts the water: ALONG **0.41x** plain, ACROSS **1.81x** (it still pays its whole bridge), a **4.5x** spread; a river too small for a hull gets no boat (**0.99x**). **`|align|` is symmetric by construction and that is load-bearing** — v1.98: an undirected Prim MST has no answer for an asymmetric cost, and a road beside a river is used both ways; measured asymmetry over 8 000 pairs **exactly 0**. **NAVIGABILITY IS A CATCHMENT AREA, NOT A STRAHLER ORDER**, and v2.59's recommendation is the reason: re-measured here, one `order>=3` label describes rivers of median catchment **6 417 km² at 800 km against 1 429 009 km² at 40 000 km — a 223x swing** in what "navigable" would mean, while the catchment is resolution-free (**483 km² at 512px, 482 at 1024px**). `CIV_RIVER_NAVIGABLE_KM2=1000` is anchored twice independently — the Thames at Lechlade / Severn at Welshpool head of navigation, and the 10 m³/s barge-discharge rule at temperate runoff (~1 050 km²) — covering **30.0% of channel against order>=3's 2.0%** (**42 -> 728** navigable edges). **The three existing `order>=3` consumers are untouched**; v2.59's nine-consumer migration is still the owner's call. **The river's worth is a SPEED from the planner's own vessel table** (`_civRiverTravelSpeed` via `jpVesselDayKm`) — the v2.34 rule applied to water; spending Diocletian's 5.5x COST ratio as a TIME ratio would be the dimensionless-number defect v2.33 removed from this same function, and a river is not uniformly quicker anyway (Longship **2.18x** walking at 5 000 kg; River Barge **0.40x** at 30 000 kg). **The planner's current was backwards one step in five** — `_jpRiverCondition` read the route's ELEVATION PROFILE, a proxy for the water's direction rather than the water's direction (**10 of 48** real steps wrong; v1.102 had already special-cased lakes out of it for the same reason). It reads the receiver tree now, asserted as a ROUND TRIP: the same 90.6 km chain reads **Strong Downstream** one way and **Strong Upstream** the other. **`cat:'river'` was structurally unreachable for a real river** — every river cell derived as `cat:'land'`, so the whole vessel/`JP_TERRAIN.river` model applied to lakes and nothing else; now **54 km of a 90.6 km chain** against 0. Real roads follow them: along-river road km **251 -> 405 (1.61x)** over three seeds with crossings flat at **0.97x**. `hash_gen1.js` vs v2.61 **ALL IDENTICAL**; road geometry is a deliberate re-baseline. Verify with `tests/perf/probe_rivernav.js` (19 assertions; hard-errors on v2.61). |
| `Cartalith v2.61 DCC test.html` | Previous DCC-line file. Owner: *"Maybe we shouldn't carve, maybe we should only paint the current line in the same color as the lakes. And make sure the lines aren't broken bits"*, then *"Then at those pits should be lakes no?"*, then *"revert the digging thing derived from the sculpt function and focus on coloring the river banks accordingly."* **The two water renderers disagreed about what water IS.** A LAKE is OPAQUE (`lakeWaterColor` returns a colour the caller writes, at a flat 0.95 shade); a RIVER was a translucent Beer-Lambert TINT at alpha `sV*0.85`, where `sV` carries the DISCHARGE MAGNITUDE as well as the coverage — so a small river painted at **38% opacity** with the hillshade showing straight through a flat-edged **2.5 km** slab. The river now writes the LAKE's colour at its own COVERAGE (**57.3% of painted cells fully opaque against 0%**), a geometric quantity with no free parameter; `kV`/`wcol` omitted ⇒ v2.60's blend character for character, asserted by reproducing it. **The antialias band is bounded by the CHANNEL, not by a pixel** — a flat 1-px edge is right in a refined tile and wrong on the coarse grid, where one tile pixel IS a cell, capping the centreline at 80% and averaging 50%, i.e. a tint again. **ONE definition of the lake colour** (it was written out in THREE: `lakeColor`, `lakeColorSampled`, the tile's inline branch). **A depression a river flows into is a LAKE, whatever the local rainfall** — `lakeRain` (0.22) is right for an unfed hollow and wrong for a TERMINAL lake, which exists *because* a river feeds it: Chad, Eyre, the Aral and the Dead Sea all fail a local-rainfall test and all are lakes. The supply term was already computed (`flowField`) and the threshold is `riverFlowThresh`, so **no constant of its own**; lakes **15 904 -> 17 725 cells**, asserted MONOTONE. **v2.60's sculpt-derived digging pass is REVERTED**: against v2.59 the battery reads `field` **3273059064** IDENTICAL on both sides in all five scenarios with only `rgba` moving — v2.61's terrain IS v2.59's, byte for byte, which proves 2c was the whole of v2.60's field divergence. **And the lines are not broken bits**: 455 of 1104 stems are isolated but their median length is 6.2 cells, so culling them all would delete 41% of the network; what is removed is what the geometry condemns — **shorter than it is drawn wide**, self-calibrating, no new constant, and safe where v2.60's refuted length-cap was not (removing a stem nothing joins cannot disconnect anything; narrowing one can, and did, 0 -> 111 breaks). Breaks stay **0**. **The banks are coloured from the band the water is already stamped in** — the palette existed in `applyCoastRiverSDFv` but was gated on `sdfRivers` (default 0) and fed by `buildRiverSDF`, **3078 ms** and a v1.29 seam risk; the band falls out of `riverFieldTile`'s own segment walk instead, free and seam-free. **REFUTED, measured**: filling the CARVED trench — at the default the terrain carries no river-scale cross-section (**wetted half-width below the 0.8-cell floor at 88.1% of vertices**), and where it does 13-37% saturate into floodplain; at 50 km, filling to the bank reaches **38-62 cells**. A name collision the RUNTIME caught: the coverage array was first called `k`, which the segment loop already uses as its counter. Verify with `tests/perf/probe_riverfill.js` (24 assertions). |
| `Cartalith v2.60 DCC test.html` | Previous DCC-line file. Owner: *"rivers are quickly small strokes one after another and when we zoom in, maybe we should paint/fill them the same way we do with lakes"*, then *"Be sure a river doesn't break up into parts. You can also use a bit of the sculpt tools to make sure a length of the river is deep enough to constitute river."* **The report was literally true and the cause was ONE NUMBER.** `buildRiverNetwork` stamps `intensity`/`depth` as a DISC AT EACH CHANNEL CELL and its `halfW` floors at 0.5, where `r=ceil(0.5)=1` means only `d===0` passes — **one cell per centreline point**. A D8 chain steps diagonally **42%** of the time and two diagonally adjacent SQUARES touch only at a corner: **4 856 four-connected components for a network of 134 rivers**, 21% of painted cells with no orthogonal neighbour. **The sibling already had the fix**: `carveRiverValleys` cuts at `halfW=0.8` at order 1, above a cell's circumradius `sqrt(1/2)=0.7071`, and **v2.30 fixed exactly this for the carve** and nobody carried it to the render stamp. Five render changes: the main map joins `riverFieldTile` via `riverMainField()`/`renderFieldCached` (v2.40 had wired the tile and both bakes, never `surfaceColor`); a second floor **`RIVER_MIN_HALF_CELLS=0.8` in GRID CELLS** (`RIVER_TILE_MIN_PX` is TILE pixels, so it shrinks to **0.014 cells** at a 9.4 km view — the symbol evaporates where you are looking); the profile becomes a **CROSSOVER** (`rIn>=wIn ? 1-dist/wIn : 1`) — v2.25's `max(symbol,real)` as a profile, and what a lake already does; the raster geometry becomes **WHOLE STEMS** (one crop held **13 polylines — one 59-point stem and TWELVE 2-3 point stubs** drawn wider than they are long: the beans and scallops; **4** after); and the raster **stops cutting at lakes** (every raster consumer paints the lake and not the river there, so the cut removed nothing visible and opened the junction — v1.29 said this itself for the GeoJSON export). **REFUTED**: capping the floor by reach length (v2.50's rule) removes the beans and takes broken stems **0 -> 111**; `rdpSimplify` at 0.25 changes the mask by **exactly nothing**, so the scallops were never the zigzag; a minimum-lake-SIZE gate moves breaks 115 -> 105. **The sculpt half** is the finishing descent pass (step 2c): **1 433 of 11 414 drawn steps CLIMB** (median 12.7 m, worst 816 m, 748 outside any lake) because the carve cuts the PRE-carve network while `_riverNet` is rebuilt from the POST-carve one — two depression models disagreeing. A full re-carve moves **5.16%** of the map for 12.55->9.88%; descent on the chain's own cells at `CHANNEL_DESCENT_CENTRE_HALFW=0.5` reaches **4.33%** and moves **0.74%**. **Read EXTENT, not stem count** (v2.30): stems 1 305 -> 1 104 because tributaries MERGE, while traced extent RISES **13 395 -> 13 907 (+3.8%)** and carved coverage **87.7 -> 91.6%**. **Headline: main stems that break into parts 884 of 1 305 -> 0 of 1 104; breaks 3 554 -> 0.** A deliberate re-baseline (default `field` FNV **3273059064 -> 1639302419**, all five battery scenarios moving), isolated: `carveRivers` off + `showRivers` off is **ALL IDENTICAL**, `carveRivers` off alone leaves `field`/`flow` IDENTICAL with only `rgba` moving. Also caught a **SEAM** (0.498 on the shared column) — the tile reject margin padded the bbox by the REAL half-width while the stamp draws at the floored one. Verify with `tests/perf/probe_riverfill.js` (20 assertions). |
| `Cartalith v2.59 DCC test.html` | Previous DCC-line file. Owner, on v2.58's disclosure that the flag is load-bearing: *"Turn it on and give me a comparison with for example strahler and if we should replace it."* **`state.hydro.integrate` defaults ON — a deliberate re-baseline of every world generated from a seed**, `field` included, because `carveRiverValleys()` cuts a real trench along the network integration changes. Measured inside ONE build at the app default: **68.5% of land drained into an interior PIT -> 0.0%**, sea-draining land **26 031 -> 42 475 cells (1.63x)**, longest whole main stem **66 -> 118 km (1.80x)**, biggest catchment at a mouth **5 078 -> 92 815 km2 (18.3x)**. `deltas` stays OFF (it deposits real sediment — a look decision). **The `loadZip` guard still defaults `integrate` FALSE on purpose**: a pre-v2.41 save carries no `hydro` block and WAS generated without it, so it must reload as the world it was (the v2.17/v2.22 convention). **Isolated**: the battery diverges everywhere, and with `integrate` forced equal on both sides it is ALL IDENTICAL BOTH WAYS (v2.58 at true gives v2.59's default FNV **3273059064**; v2.59 at false gives v2.58's **528640695**). **The Strahler comparison refuted three of my own hypotheses.** **Order is NOT resolution-dependent here** — `riverFlowThresh` is keyed on the CELL COUNT, so the mask is a constant fraction of the grid and the ladder holds at **3/3/3** across 512/1024/2048px; the classic criticism does not apply. **It IS extent-dependent**: `order>=3` covers **0.32% / 3.73% / 4.10%** of channel at 800/8 000/40 000 km — a **12.8x** swing in what "navigable" means, decided by map width. It **cannot rank inside itself** (top bucket spans **123.7x** in catchment) and is **not monotone** — in **4 of 6** configs the world's biggest river is not the top-order one, reading order 2 of 4, i.e. below the navigability gate. Max order anywhere **4**, against 8-12 for an Amazon. **The flip is the sharpest demonstration**: 68.5% of land re-routed and the trunk 1.80x longer, and order reports **3 -> 3, outlet 3 -> 3** — saturated. (v2.41's "outlet Strahler 2 -> 3" is true of the network `probe_deltas.js` rebuilds at `riverDensity:1`, not of the shipped `_riverNet`; that assertion fails identically on v2.58, so the drift is pre-existing.) **REFUTED — v2.58's stem LENGTH ranks real catchment WORSE than order**: mid-ranked rho(order) **0.379-0.734** against rho(length) **0.105-0.398**, order winning in all six. **And that corrects v2.58's own published 0.963**: it is length against `buildMainStems`' accumulation over `net.recv` — upstream CHANNEL CELLS, not catchment AREA (**0.968-0.983** there, **0.105-0.398** against the catchment raster). **v2.41's "two different trees", third occurrence, inside a figure of my own.** v2.58's fix stands (fragments vs stems is like-for-like on one quantity); the length gate is a **cartographic disclosure rule**, never an importance measure. **Spearman with order needs MID-RANKS** — a first cut read **-0.26 to +0.41** on plainly monotone data. **RECOMMENDATION: keep Strahler, stop using it where it must RANK** — catchment km2 is the extent-free continuous quantity for the `order>=3`/`order>=4` gates, a nine-consumer change and the owner's call. Verify with `tests/perf/probe_riverorder.js` (18 assertions). |
| `Cartalith v2.58 DCC test.html` | Previous DCC-line file. Owner: *"only render it when we zoom... And only do that for rivers that are actually big. Rhine, Amazon, yellow river. And do the same for smaller rivers as we do ways for the smaller cities... Attest your own research adversarially."* The research refuted two of the request's own premises and one of my own claims; what shipped is the part that survived. **River SELECTION had no scale term anywhere** — `riverRenderPolys()`'s cache key is `_fieldGen|GWxGH|minO`, no zoom and no `mapWidthKm` — while river DRAWING (Catmull-Rom, v2.25's real-width crossover, v2.40's in-tile resolution) has been mature for versions. The ladder the request asked to mirror is itself scale-blind: `CIV_LOD_PLACE.hamlet=1.4` puts a hamlet on screen at a **28 571 km** view on a 40 000 km world. The gate is **`len*_z`, the stem's own length in SCREEN PIXELS** — `mapWidthKm`-free by construction, since `_z` is screen-px-per-grid-cell in BOTH camera conventions. `RIVER_MIN_SCREEN_PX=20` sits inside the **13.08-490.6 px** band real stylesheets use (I first cited OpenMapTiles' 26.165 px as verified and had **extrapolated two rows of its table rather than read it** — the probe asserts the band, not the number). **Töpfer & Pillewizer is REFUTED for hydrography** (predicts 49% flowline retention 1:24k->1:100k; USGS measured **10-11%**), and **Strahler cannot carry these tiers** — max order **4** at world extent, **2** at the default, against 8-12 for a real Amazon. Length is the gate, and it is legitimate only because the geometry changed: `traceRiverPolylines` returns FRAGMENTS whose length ANTI-correlates with importance (rho **0.207**, top-100-by-length vs top-100-by-flow overlap **2%**, median length 141 km by flow against 1 341 km by length). `buildMainStems` accumulates drainage area over `net.recv` **itself** by Kahn's algorithm and keeps the largest tributary at each confluence — rho **0.207 -> 0.963**. **v2.59 CORRECTION: that 0.963 is length against upstream CHANNEL CELLS, not catchment AREA** (0.105-0.398 against the real catchment raster) — fragments-vs-stems is like-for-like and stands, but the length gate is a cartographic disclosure rule, not an importance measure. Two defects in my own first cut: ranking `net.recv` chains by `flowField` (**two different trees**, exactly as v2.41 recorded — stems died after 5-10 steps at flow 163 405), and a raw `Math.hypot` charging a wrapped receiver a full map width (longest 'stem' **41 097 km = 2 104 cells on a 2 048-cell grid**, and since the gate RANKS on length every seam-crossing river was promoted to the top). Longest stem now **4 879 km** (Amazon 6 400). The spline's own geometry was GW-keyed with no zoom term — **111 km** control points and a **1 000 km** meander wavelength at 40 000 km, constant at every zoom; `step`/`eps` divide by `_z` (**bit-identical to v2.57 at `_z=1` at GW 1024/2048/4096**, refining **2.844 -> 0.356 -> 0.044**) and `wl` is real-km (`RIVER_MEANDER_WL_KM=20`, **reproducing `GW/40` exactly at the app default extent**). **Eighth occurrence of the real-km defect.** `drawRiverWays` also re-traced the whole network every call (**32 ms**) while `riverRenderPolys` cached the identical work; `mainRiverStems()` uses the same key. Ladder at 20 px, LOD 0-8: **975/2531/4426/6087/7215/7456/7456/7456/7456** — monotone, 13.1% disclosed at world scale, **saturated by LOD 5**, which refutes the request's own "LOD 7/8" premise. **`state.hydro.integrate` was DEFAULT OFF and load-bearing** (longest stem 1 799 km off vs 4 879 km on) — v2.59 flipped it on the owner's call. `hash_gen1.js` vs v2.57 **ALL IDENTICAL**. Verify with `tests/perf/probe_riverscale.js` (17 assertions). |
...
### Nothing the settlement draws sits on water, and woodland is an area (v2.71, DCC-line file only)
...
passed every synthetic fixture — *"within the actual cartalith generation it seems to miss. Do we
need an extra technical layer to help guide settlement generation or can we use an existing
layer?"* `tests/run.sh` **1280/0**, `run_um.sh` **882/0**, `hash_gen1.js` vs v2.70 **ALL
IDENTICAL**; verification is `tests/perf/probe_woodland.js` (10 assertions; pass v2.70 as a second
argument for the control half).

- **The answer to the owner's own question is NO NEW LAYER — the mask already exists and simply
  never reached the renderer.** On the REAL map-water path `buildSite` sets **`waterPoly = []`
  deliberately**, and its own comment says why: the map already draws the sea beneath the town, so
  the town must not paint a second one. That is right for FILLING, and it leaves every renderer with
  **no water geometry at all** on exactly the towns the app generates — **7 of 39 real towns carry
  an empty `waterPoly`** — which is why a clip keyed on it passed every fixture and did nothing
  live. `_umWaterCtx` has always built a 22 m mask of the real sea, lakes and river band and
  **`isWater` reads it already**; it never reached the model because `cityGen`'s site record is
  function-free and the mask was not among the fields copied. `landRuns` is that mask, run-length
  encoded, carried across: **38 of 39**.
- **Run-length rectangles, not a traced contour.** A 22 m stair-step is finer than the coarse grid's
  own ±half-cell uncertainty about where the coast is (v2.60), so marching squares would be eighty
  lines to make a boundary smoother than the data underneath it. The four corners of each run go
  through `gxy`, so the clip inherits the layout's rotation and the camera — a rect in screen space
  is axis-aligned and wrong the moment the town is turned.
- **ORDER IS LOAD-BEARING: the mask beats the polygon**, and preferring the polygon was a real
  defect in my own first cut. A real-water town carries BOTH — 54–113 land runs alongside
  `waterPoly`s of 4, 6, 10 and 14 points built from a river centreline — so the polygon clips the
  town against a sliver of one channel and leaves everything else standing on open sea. The mask is
  what `isWater` itself reads, so it is the authority; the polygon is the synthetic path's fallback.
- **A CLIP, NOT A GEOMETRY FIX, and the measurement is why.** ZERO blocks, parcels, buildings or
  fringe parcels have a sample inside the water, and ZERO street CENTRELINES do either. What crosses
  is the street's **WIDTH** — stroked ~8.4 m with its casing — and `removeWaterCrossings` samples
  centrelines only. Insetting streets leaves blocks, walls, buildings and every future feature to be
  fixed one at a time; the clip states the requirement once, for all of them, this version's own
  woodland included. Released for the bridges and the ford, which are MEANT to span water.
- **The fringe needed its OWN clip span.** It must stay drawn BEFORE the water fill (v2.68: where a
  strip clips a channel the water wins) — but where there is no water fill, "drawn under the water"
  stops being a backstop and a field or a wood stands uncovered on the sea. Two real towns kept 24
  and 4 encroaching pixels after the built fabric was clipped, and both were fringe.
- **TWO CHEAPER MEASUREMENTS BOTH LIED, and that is the most reusable thing here.** Counting
  ...
  like antialiasing, because the denominator is a 211 000-pixel sea while every wrong pixel is in
  the one place the eye goes. Counting pixels whose colour matches a built palette misses the
...
- **And one of my own visual calls was simply wrong.** Stroking the waterline over the render showed
  magenta running through the shore blocks, which read as an overhang — it was the 1.5 px stroke
  straddling a street that stops exactly at the line. The pixel diff was right and the eyeball was
  not: the coast fixture had **zero** encroachment all along, and the real one was `riverthrough`,
  where a primary may cross open water because `removeWaterCrossings` exempts primaries as presumed
  bridges — safe for a synthetic single-channel site, and that town had **0 bridges and no ford**.
  Measured on a real generated world, before → after: **36 → 0, 61 → 0, 24 → 0, 1 → 0, 4 → 4,
  0 → 0**; control worst **61 px against 4**. The residual is clip-boundary antialiasing.
- **WOODLAND IS AN AREA, because a point scatter cannot make a mass.** The engine has scattered
  `kind:'tree'` points since v0.95, and no density of stipple produces an edge. `buildWoods` emits a
  polygon and puts a handful of trees INSIDE it as texture ON the mass rather than as the mass:
  **9–10 woods on every site kind and size**, none on water, none on a field.
- **The plough wins, and `farm` is an input for that reason.** A wood is rejected where a field
  already sits, never the reverse — the historical order, since assarting clears woodland FOR
  fields, so the arable is the later, winning layer.
- **NO `crossesStreet` guard, and that is the difference between a wood and a field.** The farmland
  generators reject a candidate the moment a street crosses it, because a furlong with a road
  through it is not a furlong. A wood with a road through it is an ordinary wood.
- **THE FRAME WAS WRONG BEFORE IT WAS MEASURED, TWICE.** Keyed on `maxRF` the candidate band ran to
  ...
  two: **672, 687, 688, 694, 724, 745, 750 m** from the market, a ring one wood thick that reads as
  a hedge around the map. Loosening the farm clearance and dropping the street guard each moved the
  count by **zero**; only reframing the search on the site BOX did. **Guessing at a constraint twice
  and measuring once is the wrong order, and the measurement was cheap.**
- **A probe assertion was CORRECTED, not loosened.** `probe_fringe`'s hedgerow gate counted every
  fringe fill as its denominator, and woods stroke in their own colour — so it read 98 hedgerows
  against 108 fills and went red. Scoping the denominator to the field/pasture colours passes on
  **v2.70 as well**, which is what makes it a correction rather than a threshold moved to suit the
  new build (v2.67's rule).
- **Known scope cuts**: the woods band wherever the field fans leave room, which on a road-fanned
  ...
  as a belt rather than as scattered copses; no clearings and no ponds inside a wood; woods have no
  relationship to the real biome on the real-terrain path, so a desert town still grows them; and
  the clip's residual is one antialiased pixel along the waterline, inherent to clipping.

### One colour chain, four surfaces — the village-map preset (v2.70, DCC-line file only)

Owner: *"And for the coloration and style can we add it as a preset in the styles section under
carto."* Verification is `tests/perf/probe_villagestyle.js` (14 assertions).

- **Check the size of a feature before building it** (v2.58). `landColorCore` is the ONE land-colour
  function and has four call sites — main map, LOD tile renderer, flat bake — and already ends in a
  chain of per-pixel NPR steps each gated on its own `state.viz` key. `seaColorCore` is its twin.
  So a palette is one more step in each chain plus one key, one preset row and one button, and
  every surface that draws a map gets it. **The probe asserts the tile and the bake specifically**,
  because that is the claim the design rests on and a canvas-level tint would pass everything else.
- **Quantise the LIT colour, not the biome.** It has already absorbed slope, aspect, material and
  shading, so quantising it keeps every distinction the map had and removes only the gradient —
  which is the whole difference between a relief map and a drawn one. Sea keys on DEPTH instead,
  because a flat body with one lighter shelf band is a depth fact, not a lighting one.
- **`shadows:0` is load-bearing.** The hillshade runs BEFORE the ramp, so leaving relief shading on
  puts the gradient straight back into the look whose point is not having one.
- **Apply a replacement at the END of `seaColorCore`, not a mix into `wc`.** By that point the
  seabed grain, the bathymetric hillshade and the smooth depth ramp are all folded in; mixing
  earlier leaves all three showing through.
- **TWO OF THE PROBE'S OWN ASSERTIONS WERE WRONG FIRST, and both generalise.** (1) Reading the
  canvas straight after `_applyStylePreset` returns the PREVIOUS frame, because it ends in the
  deferred `render()` — force `renderNow()` before any pixel comparison. (2) Counting EXACT distinct
  colours read **1322 → 1322** against a screenshot that plainly showed the change, because an 0.88
  blend leaves 12% of the original carrying the noise; **count quantised buckets when the claim is
  "collapses onto a small palette"**, because that is what the claim actually means.
- **Two navigations to reach a control is correct; zero clicks reaching it is not.** The row sits in
  a category accordion inside the Cartography panel, so the probe opens the domain from the real
  rail button and the accordion from its own `<summary>` — found by walking UP from the button
  rather than by a hand-written selector that stops describing the shell the moment it is
  rearranged — and only then asserts the click LANDS (v2.46).
- **A NEW MANAGED KEY MUST BE DECLARED IN THE `state.viz` LITERAL, and the suite already knew.**
  `village` went into `STYLE_MANAGED_NUM` without being declared beside `cel`/`sepia`/`risograph`/
  `pointillism`, so v0.63's own *"style preset probe restored managed viz keys"* assertion — which
  snapshots every managed key and checks the restore — read `undefined` instead of 0 and went red,
  taking a downstream assertion with it on a re-run. **A key a preset can write but the state
  literal never declares is a key that does not survive a save**, which is exactly what that
  assertion has been guarding. Add the declaration in the same edit as the `STYLE_MANAGED_NUM` entry.
- **Known scope cuts**: the civ layer's ways, territory fill and labels keep their own colours;
  lakes follow the sea through `seaColorCore` but the river overlay's stroke does not; the ramp has
  no control beyond the one `village` slider — a user-editable palette is a different feature.

### Refinement must not move the coastline (v2.69, DCC-line file only)

Owner: *"when I zoom in and the LOD renders deeper it often happens that the coastline, or rivers
for that matter, don't align properly with the settlement."* Verification is
`tests/perf/probe_lodalign.js` (11 assertions; pass v2.68 as a second argument for the control
half), plus v2.47's own `probe_lodsurface.js` **12/0**.

- **Reproduce and DECOMPOSE before fixing — the decomposition picked the fix, and it was not the
  first guess.** `burnChannels` and `featureDetailPass` are the obvious suspects and **neither runs
  at the app defaults** (`coarseFlow`/`coarseOrder` are null unless "Burn rivers" is on); v2.52's
  crater registry does run and makes **exactly zero** difference. Measured share of a refined
  tile's pixels drawn as the opposite of what the settlement's surface says: **z2 0.40%, z4 0.86%,
  z6 5.73%, z8 8.56%** — monotone in depth, which is the report in one line.
- **THE DOMINANT CAUSE WAS THE FILTER.** `_umWaterCtx` decided the town's coastline with a
  hand-written bilinear sample; **v2.47** moved the LOD tile's reconstruction to `sampleC1` and
  nobody carried it to the adapter. **17× the disagreement** (2.89% vs 0.17% with the filters
  matched). `sampleC1` is *interpolating*, exact at coarse nodes, so this is the same surface —
  not a smoothed one. **When you change how a surface is reconstructed, grep for every other
  function that reconstructs it.**
- **THE SECOND CAUSE IS A ONE-SIDED TAPER.** `amplifyRegion` fades detail out going DOWN from the
  shelf (`underwater`) and does nothing going UP, so a land pixel a hair above sea took the full
  ±`detailAmp`/2 band: **2.31% land→sea against 0.30% the other way, a 7.6 : 1 asymmetry, worse at
  every level**. **v2.40's rule — refinement adds resolution, it does not invent** — and the
  land/sea boundary is a decision placement, `_umWaterCtx`, `_civLakeFlooded` and the road network
  are every one of them built against.
- **Clamp the DELTA, not the result.** Clamping `v` to `sea` pins a whole coastal band to one
  value and makes a flat shelf — **v2.50's "a floor that INFLATES is not a bound", in a renderer**,
  trading a wandering coastline for a terraced one. Capping the excursion *toward* sea level at
  **half the remaining headroom** needs no constant, can never reach the far side, and is confined
  by construction: a pixel whose detail is under half its own headroom is bit-identical.
- **THE FIRST CUT GUARDED THE WRONG FUNCTION, and only the control caught it.** The detail band is
  added in **two** places — `amplifyRegion` and `addZoomDetail` — and clamping the second alone
  left 3.33%. The probe reported *"control 1158 drowned, fixed 1158"*: an identical number on both
  sides, which reading the diff would never have produced. **A control that must differ is what
  turns a fix into a measurement.**
- **After**: **z2 0.35%, z4 0.10%, z6 0.28%, z8 0.13%**, worst level **8.56% → 0.35%**, **0 of
  41 984** pixels drowned against 1 158, inland detail unchanged at **4.91e-3**, seam Δ still
  **exactly 0**.
- **A deliberate, bounded re-baseline of two things, neither of them `field`**: LOD tiles and baked
  atlas chunks within one detail-amplitude of sea level (a stale atlas wants a re-bake), and
  generated town layouts (the water mask is part of `_umCacheKey`). The default render never
  reaches `amplifyRegion`, and the LOD overview calls it at `detailAmp:0` where the clamp is a
  provable no-op.
- **DISCLOSED, MEASURED, NOT FIXED — the river half is a DIFFERENT defect, and it is a WIDTH
  mismatch rather than an offset.** The centreline is shared by construction (`_umWaterCtx` reads
  `_civRiverPolylines()`, the same trace the renderer draws). The width is not: `_umWaterCtx`
  derives it as `10 + order*7` capped at 46 m — its own formula — while **v2.49** gave the renderer
  a real hydraulic half-width in `_riverNet.halfw[]` and recorded in the same breath that both
  river renderers "were built for this and both receiving a constant". Measured at the app default:
  **order 1 — map 155 m vs town 17 m (9.1×); order 2 — 452 vs 24 (18.8×); order 3 — 764 vs 31
  (24.7×)**. **CORRECTED AND CLOSED**: those figures compare the town to the RAW `halfw`, which is
  not what the map draws — the drawn width also carries v2.60's `RIVER_MIN_HALF_CELLS = 0.8`, a
  floor in GRID CELLS, so at the app default **every order draws at 2500 m against a 1700 m town
  box**, the order distinction collapses completely, and the true ratio is **~104×**. **The owner's
  decision is to LEAVE IT, because both numbers are honest**: v2.60's floor is a RESOLUTION
  statement — the centreline is known to ±half a cell, so a narrower channel would claim precision
  the data does not carry — and at 800 km one cell IS 1.5 km, so the map genuinely cannot locate a
  30 m river. The town's 24 m channel is the physical truth; the band is the positional one. Every
  way of forcing agreement costs more than the disagreement (narrowing re-baselines every LOD tile
  and risks v2.60's measured 0 → 111 broken stems; widening drowns a 1700 m village in a 2500 m
  channel; suppressing the band inside a town's box helps only where a town is drawn). **A known
  limit of a 1.5 km cell, not an open defect.**

### The fringe was generated and drawn nowhere (v2.68, DCC-line file only)

Owner, on a reference village map: *"tell me what features you see"*, then *"Can you create these
features in cartalith"*. `tests/run.sh` **1280/0**, `hash_gen1.js` vs v2.67 **ALL IDENTICAL**;
verification is `tests/perf/probe_fringe.js` (13 assertions; pass v2.67 as a second argument for
the control half). `run_um.sh` **882/0** — block 4 is untouched.

- **Check what a feature already produces before building it** (v2.58's rule). The inventory the
  request came from listed ~25 features and the single largest block of them — strip fields,
  pasture, orchards, wells, a market cross, yard fences, scattered trees — were **already
  generated**. `buildFarmland` has pushed `field`/`pasture` polygons into `model.details` since
  v0.95; neither map renderer reads `model.details` at all, and the City Viewer's detail pass
  branches on nine kinds with **no branch for either**. So the work was a renderer, not an engine.
- **A lookup that silently skips an unknown kind hides a whole feature** — v2.65 found exactly
  this in `_UM_DISTRICT_FILL` one layer down, and the same shape was sitting in the detail pass
  beside it. v1.17's own known-scope-cut line ("`model.details` never drawn by the Gen1
  renderers") was written about all of them and only ever half-closed by v1.18.
- **MEASURE THE THING YOU ARE ABOUT TO DRAW.** The reference shows furrow hatching whose direction
  changes per parcel, and the obvious build is a clipped hatch plus a stored plough bearing.
  `stripFields` already cuts each grant at **5.8 x 91.5 m, every polygon a quad, aspect 15.8:1** —
  a **selion**, one plough-run, and a furlong is a bundle of parallel selions. Over 98 parcels:
  **10 distinct bearings, the largest bundle 52 parcels sharing one**. The texture and the
  orientation are the geometry, so **no hatch pass is written and no bearing is stored** — either
  would be a second way to say what the polygon already says. The probe asserts both halves,
  because if the selions did *not* carry an orientation then storing one would be the right fix.
- **A gate must be asserted in BOTH directions.** The hedgerow is keyed on the parcel's **own**
  shortest edge, not a global zoom threshold: a 5.8 m strip drawn 0.6 px wide with a 0.5 px dark
  outline reads as the outline, so the fringe comes out one dark wash — **v2.50's "a floor that
  INFLATES is not a bound", in a renderer**. 98 fills / **0** hedgerows at 0.08 px/m, 98 / **98**
  at 1.0. A gate that never fires is the same as no gate; one that always fires is the defect.
- **The fringe is GROUND, so it is drawn FIRST — under the water.** It spans **1726 x 401 m
  against a 187 x 332 m built mass**: it is the country the town sits in. `stripFields` already
  rejects on three water sample points, and drawing under the water means a strip whose far end
  still clipped a channel loses rather than painting over it. The probe asserts the ORDER.
- **ONE helper, three renderers.** `_umDrawFringe`/`_umDrawFringeProps` serve `_umDrawLayout`,
  `_umDrawLayoutPreview` and `_cvDrawCity`. Trees and yard fences **MOVED** out of the City
  Viewer's `CV_LOD_MAX` glyph pass into that helper rather than being written a second time — they
  were invisible until full zoom there and invisible at every zoom in the other two. Each prop
  gates on **its own** size reaching half a pixel (**28 trees -> 0 marks at 0.02 px/m, 28 at 1.0**).
- **The control half is what makes this a measurement.** "It draws something" passes on a build
  that draws a tone for another reason; the probe opens v2.67, confirms the identical 98 parcels,
  and confirms **0 drawn in all three renderers** (v2.39/v2.57).
- **Known scope cuts, measured**: the strips do not **tile** — `stripFields` spaces grants
  **28-40 m apart at 4-7 m wide**, so the fringe reads as separated slivers against the
  reference's contiguous fabric; closing that is a generator re-baseline with its own measurement
  and would spend the guarantee that makes this version checkable. It also skips anything within
  **330 m of the market** (the near fringe is deliberately empty), and `FARM_SPEC` carries only
  `medieval` (strip) and `venus` (ring), so every other profile generates no farmland at all.
  Of the items this list called genuinely unbuilt, woodland as an **area** shipped in v2.71 and
  the village green + footpath class in v2.73. Still absent: roof treatment by building function,
  detached outlying farmsteads, and the shore's sand band and shallow-water shelf.

### The ward sets the plot grain, and the subdivision is what it sets (v2.67, DCC-line file only)

`tests/run.sh` **1280/0**, `run_um.sh` **882/0**, `hash_gen1.js` vs v2.66 **ALL IDENTICAL**,
`probe_typepreview.js` 41/0, `probe_industry.js` 29/0; verification is
`tests/perf/probe_wardgrain.js` (23 assertions; pass v2.66 as a second argument for the
bit-identity half).

- **Two functions were answering one question, and one of them could only say "near" or "far".**
  `buildParcels` asked "core or fringe?" with a hardcoded `dM<160`; `assignDistricts` asked the
  same question 130 lines later with the plaza, the wall ring, the river, the quay and the market
  radius, and produced **seven wards**. `makeWardAt` is that chain extracted VERBATIM and built
  once in `cityGen`, so the plot grain and the district labels cannot describe different towns.
- **Measure the half you are about to build before you build it.** Ward-driven DEPTH is the
  obvious feature and it would have been **v2.42's defect**: **67.6% of parcels never reach
  `depthTarget`'s own 14 m floor** — the block's waist (`tMin*0.42`) binds, not the draw — and
  tripling `plotDepthVariance` 0.22 → 0.60 moves median depth **11.09 → 11.07 m**. That is also a
  standing finding about one of v2.66's own 22 sliders, and the probe pins it.
- **So the ward drives the SUBDIVISION**, which is what the task names and what is live. `splits`
  was `min(subdivisionCap, floor(age/3))` — **street age alone** — and the halving chance was a
  flat `0.4`, so a market frontage and an agrarian-fringe frontage of the same age subdivided
  identically. M-PAR-1's register row says mature widths come from *"grant + subdivision history
  (p_split per epoch ≈ 0.1–0.2)"*, and what selects a plot for halving is the VALUE of its
  frontage.
- **Mean frontage per ward is NOT a function of the pressure alone, and a first cut of my own
  assertion assumed it was.** It demanded monotonicity across the wards ranked by pressure and
  failed by 0.08 m, because the BLOCKS differ per ward (harbour blocks are small quay-side strips,
  agrarian ones broad fringe faces) and that geometry is already in the baseline. **Compare a ward
  to itself**: market **−1.36 m**, burgher −0.68, harbour −0.53, artisan +0.03, suburb **+1.57**,
  agrarian **+1.60**; the core-to-fringe gap **0.88 → 3.85 m**; market gains parcels off the same
  streets (1 528 → 1 721) while agrarian loses them (419 → 378).
- **No `WARD_GRAIN` row is deeper than 30 m** — the deepest base the retired proxy could already
  choose. The depth column REDISTRIBUTES inside an envelope the engine already produced rather
  than extending it, because depth buys nothing (above) and a deep narrow plot is exactly what
  spans a channel (below). **An assertion of mine caught the table still at 34 m after I had
  decided that; the table was wrong, not the assertion.**
- **`wardGrain:0` is bit-identical by construction** — `1+(k−1)*0` is exactly 1, `age/3*1` and
  `0.4*1` are exact, and the `rB` draw count is unchanged — so the guarantee is structural rather
  than checked (v0.98/v2.63). Asserted against v2.66 over 30 towns: **0 divergent**. The shipped
  default is a measured re-baseline: **441 of 20 316 parcels (2.17%)**.
- **THE CORNERS ARE NOT THE FOOTPRINT EITHER.** `buildParcels` has tested only the four corners
  since v0.95, and a plot spanning a NARROW channel has every corner on dry bank: the goldens
  caught one **craftriver parcel 41.3 m deep on a 5.8 m frontage** with its middle in the river.
  **Pre-existing and unreachable** while every block platted at one grain — proven by measuring
  v2.66 at `plotDepthVariance` 0.60, where depth reaches the 46 m clamp: **zero wet parcels**, so
  widening the test rejects nothing on the old path. Seven sample points, not a polygon
  intersection, which would cost a real test per candidate in the hottest loop here.
- **`par.ward` is the PLOT SERIES' ward and `par.district` is the parcel's final label** — they
  are deliberately different fields, because the economy and status passes re-tag parcels after
  the grain was already chosen (a market-block parcel can end up a tan yard). Only the block ward
  had any say in how the frontage was cut.
- **`tests/run_um.sh`'s assertion TOTAL is not a constant** (852 → 882 here) — its Venus waterway
  check is per-BUILDING. **Quote it as "0 failed", not as a count** (v2.32 recorded the same thing
  about `tests/run.sh`).
- **A v2.66 probe assertion was CORRECTED, not loosened**: its parcel-spin check tested one claim
  with two bounds, one an absolute parcel count calibrated on v2.66's own layouts. The corrected
  form (a second SHARE) passes on **v2.66 as well**, which is what makes it a correction.
- **Known scope cuts**: realised plot aspect is **1.09–1.74** against M-PAR-2's 1:3–1:10 burgage
  band, and the same measurement says why — that is a `buildBlocks` sizing question, disclosed not
  fixed. So §1.1 #22's *"deepest plots at quay"* still does not land (harbour mean depth 10.7 m,
  the shallowest in the town) while its *"narrowest frontage of any family"* does. A block
  straddling a ward boundary takes one ward — the parcels do not exist when the grain is chosen.

### A menu that previews a settlement TYPE from its parameters (v2.66, DCC-line file only)

Owner: *"Design a proper menu to generate previews for settlement types based on parameters. See
the proof of concept settlement generation for an example."* `hash_gen1.js` vs v2.65 **ALL
IDENTICAL**; verification is `tests/perf/probe_typepreview.js` (41 assertions) plus
`tests/run.sh` 1280/0, `run_um.sh` 852/852 and `probe_industry.js` 29/0.

- **Most of this was already built, and finding that out is what decided the design** (v2.58's
  rule). Block 4 has exported **`DEFAULT_RULES` — 22 named generation parameters across street /
  parcels / settlement** — plus `resolveRules`, `applyWildness` and `applyPlotChaos` since v0.95,
  and `cityGen` reads `opts.rules` on its very first lines. **`_umPlaceContext` has never set
  it.** So every town this app has ever drawn came out at `DEFAULT_RULES`, and there was no
  control anywhere: **v2.63's shape one layer down** — a table of numbers, each declared once,
  each hardcoded, with no UI at all. The preview half existed too: `_umDrawLayoutPreview` takes a
  bare model and fits it to its own built mass, and `cityGen` runs against a SYNTHETIC site (no
  water, no terrain, no roads), which is the path `tests/run_um.sh`'s 852 goldens exercise. What
...
- **A fourth pane in v2.63's own screen, not a fourth window.** Global → **Types** → Factions →
  ...
  ✕/Escape contract and the `aria-pressed` pin all already existed — so there is no second way in
  and no second way out, and v1.57/v2.46 hold by construction rather than by care.
- **What is SAVED per type is the rules object and NOTHING else.** Site, culture, specialisation,
  walls, population, age and seed steer the preview and are deliberately not stored, because each
  already has a per-settlement home (`p.umWalls`, `p.specialisation`, `p.pop`, `p.umAge`,
  `civFactionCulture`, `_umSiteKindFromTerrain`) — a second per-type copy of any of them is
  v1.57's one-control-two-surfaces defect. The rules object is the only thing with nowhere else
  to live, which is exactly why it is the thing that is saved.
- **Bit-identity is STRUCTURAL, and a glance must not spend it.** `state.civTypeRules` starts
  `{}` and **`civTypeRulesFor` returns null rather than an empty object**, so `rules` is simply
  ABSENT from the context and `resolveRules` takes its `DEFAULT_RULES` branch. Only a real edit
  writes, and the probe asserts that **rendering the pane stores nothing** — otherwise opening the
  screen to look would author every type it touched.
- **Assert the EFFECT, and assert it as an ORDERING** (v2.42 / v2.64). A slider that stores a
  number nothing reads is the defect this pane exists to close, and "the model hash moved" is
  satisfiable by noise. So the load-bearing assertions compare two presets whose rules differ in a
  KNOWN direction on one seed: Medina's `deadEndBias` 0.40 leaves **more cul-de-sacs** than
  Planned Grid's 0, and its `subdivisionCap` 4 cuts **more parcels** than Planned Grid's 1.
- **The rules reach the MAP, which is what stops this being a toy**, asserted by generating a real
  placed town before and after and comparing `UME.hashModel`. **`_umCacheKey` had to carry them**
  or a stale layout survives the edit for ever and silently — v1.28's `_assetGen` lesson, in the
  one cache that answers "what does this town look like".
- **Every list is built from the table that defines it**, and the probe asserts each COUNT against
  that table rather than against a hand-written number (**v2.65: a hand-list of things to check is
  the same defect as a hand-list of things to define**). The same discipline pins two things a
  hand-list would not have: every `UM_RULE_SPECS` key names a real `DEFAULT_RULES` field (or a
  slider writes into nothing), and every default sits inside its own slider range (or the control
  opens clamped).
- **The preview reports what it generated.** Buildings / blocks / parcels / km of street / wall
  style and gate count / generation time, plus the quarter tally — read off the model, so a change
  that moves nothing reads as a change that moves nothing.
- **The first layout was wrong and only a SCREENSHOT said so** (v1.44's precedent). The shell caps
  ...
  under it.
- **A preview has no map, so it names what it is assuming.** The prevailing wind and the river's
  direction are FIXED constants here and the pane says so in words, because v2.64's industry
  siting reads both and omitting them would silently drop four quarters a real town has. v1.35's
  `basis` rule.
- **Three more engine options are exposed as preview-only, disclosed rather than silently inert**:
  `cityGen` reads `opts.faith`, `opts.civicStyle` and `opts.harbourDefence`, and
  `_umPlaceContext` sets none of them. Wiring each to a settlement needs a per-settlement field
  (faith is really `civFactionReligion`'s job) and is its own decision.
- **Exposing the table reached a NON-TERMINATING REGION of the engine's own documented parameter
  range — the most important thing in this version.** `buildParcels` lays frontage grants along a
  block edge by drawing a width and RE-DRAWING whenever it does not fit the remaining frontage,
  with no upper bound. When the remainder sits just above the 4.5 m floor but below a typical
  grant, the only escape is the lognormal's far lower tail, and the expected retries explode as
  `frontageWidthVariance` falls: measured against a 4.6 m remainder, **0.40 escapes in ~68 draws,
  the 0.22 DEFAULT in ~28 571, 0.18 in ~2 000 000, and 0.12 — the PoC's own 'Planned Grid'
  profile — effectively never.** Latent for the life of the file because nothing ever set
  `opts.rules`, so every town ever generated ran at 0.22; reachable the instant a type's rules are
  editable. **Found by bisection, not by reading**: all nine street parameters measured ~450 ms
  alone at pop 900, and the parcels group alone did not return.
- **The fix is a BOUND, not a new formula — and the first claim made for it was wrong, which is
  what measuring it caught.** `PARCEL_GRANT_MAX_SPIN` caps consecutive non-placing iterations, and
  the obvious thing to say is that it cannot change a terminating run. **It can, and it does.**
  `run_um.sh` is 852/852, so the goldens' own fixtures never reach the bound — but an ordinary
  default-variance town does: the true maximum over twelve towns is **172 644** spins against a
  bound of 4096. So this is a **deliberate, bounded re-baseline of generated town layouts**, sized
  by measurement rather than argument: raising the bound to 2^22 and regenerating the same twelve
  towns, **10 of 12 differ, by at most 10 parcels (1.08%), 46 of 8 939 overall (0.51%)** — the
  last grants of a block edge, never the town's character. `UME._test.setParcelGrantMaxSpin`
  exists so that comparison is possible and is not reachable from the app. **A probe that only
  asserted "the goldens still pass" would have reported a byte-identity that does not hold.** What
  the loop was deciding is worth saying plainly: whether to squeeze one more 4.5 m grant into the
  last ~4.6 m of an edge, for tens of thousands of draws. All six presets × eight populations from
  900 to 20 000 now return in **440–930 ms**, where Planned Grid did not return.
- **`hash_gen1.js` vs v2.65 is ALL IDENTICAL**, because urban layouts are opt-in and the default
  render never reaches this code — the re-baseline is confined to generated town layouts.
- **Known scope cuts**: no named/saved rule profiles beyond the five carried over from the PoC (a
  type IS the profile here, and the project zip is the save path); the synthetic preview site has
  no real terrain, so `terrainAware` is off there and a type cannot be previewed against a
  particular piece of ground; `civTypeRules` joins `GEN_PARAM_BLOCKS` so the dump carries it, and
  — like `civParams` — an import only populates keys the reference state already has (v2.54's
  "bounded by the reference").

### The status gradient, made explicit — and the second palette (v2.65, DCC-line file only)

`docs/05` §7.3's closing bullet, plus a gap of v2.64's own. `tests/run.sh` 1280/0, `run_um.sh`
852/852, `hash_gen1.js` vs v2.64 ALL IDENTICAL; verification is `tests/perf/probe_industry.js`
(29 assertions).

- **"Already implicit" is not the same as available.** Density and centrality carried the status
  gradient, and nothing could read it. `par.status` is that number made explicit, out of quantities
  `assignDistricts` already had — no new pass, no new constant, written on every parcel.
- **Assert that a gradient is a GRADIENT, not that a label exists.** 0.662 near the market against
  0.218 at the edge; outer ground 0.143 downwind against 0.403 upwind. §3.1 summarises the poor
  quarter as "to the edge AND downwind" — both terms live in the one number, so choosing it is one
  sort rather than a second rule.
- **TWO palettes, and v2.64 fed one of them.** `_UM_ECON_TINT` is building fill; `_UM_DISTRICT_FILL`
  is the City Viewer's PARCEL fill, where `if(!fill) continue` skips an unknown district in silence.
  v2.64's four districts had tints and no fills, and **its own probe asserted only the palette it
  had remembered** — the invisible-feature defect (v1.80/v2.15) landing one layer down.
- **A hand-list of things to check is the same defect as a hand-list of things to define.** The
  assertion now covers every district actually OBSERVED on a parcel, and that change alone found a
  PRE-EXISTING hole: `buildFaithSites` has always tagged precinct parcels `church` and this palette
  never held one (confirmed absent on v2.63), so a cathedral close drew with no fill at all.
- **Known scope cuts**: §3.1's institutional quarters (cathedral close as a precinct, castle bailey,
  Jewry by the castle, the foreign factory at the quay, friaries in the suburbs, monastic precincts,
  hospitals at the gates) each need a building or precinct to anchor on — a larger piece than a
  gradient, not started. `par.status` feeds district choice only, not building grammar or parcel
  grain; that is the `buildBlocks`→`buildParcels` seam.

### Industry sits where its driver puts it, and the driver is real (v2.64, DCC-line file only)

`docs/05` §7.1's two site-model vectors, shipped with their first consumer (**M-IND**, §4.7).
`tests/run.sh` 1280/0, `run_um.sh` 852/852; verification is `tests/perf/probe_industry.js`
(23 assertions); `hash_gen1.js` vs v2.63 ALL IDENTICAL.

- **Ship a field with its consumer.** The wind and downstream vectors alone would be two fields
  nothing reads — v2.14's dead-code rule. M-IND is what makes them checkable by effect.
- **Do not invent a direction a field already knows.** Wind is `currentWindField()`; downstream is
  v2.62's `_civRiverFlowField()`. Both return BEARINGS with `orient` subtracted, which is
  `_umOreBearing`'s own convention — a second frame convention would be the drift this file keeps
  paying for. Null on failure, never a fabricated direction (v1.35's `basis` lesson).
- **`_civRiverFlowField` fills `km2` everywhere and `fx`/`fy` only on CHANNEL cells.** Selecting the
  nearest cell by `km2>0` therefore lands on the town's own dry ground: **1 of 14 towns got a
  bearing**. Select on the vector — **14 of 14**. *Read what a field is populated on before
  searching it.*
- **"Downstream of the market" is unsatisfiable for most towns, and the measurement is why.** The
  obvious §4.2 predicate gave **ZERO tan yards across 14 towns** while its mirror gave mill races;
  one town held **62 riverside parcels, all 62 upstream of its market**, because a river clips the
  box on one side. What §4.2 describes is the downstream END of the town's own frontage — an ORDER
  along the flow, needing no origin. **v2.62's lesson in a second subsystem: the direction is what
  is real; the origin is not.**
- **Assert the ordering, not the sign.** Within one town every tan yard must sit downstream of every
  mill race. A frontage entirely on one side of the market satisfies no sign rule and must still
  satisfy this one — and it can only hold if the vector is genuinely consulted.
- **A siting driver is a property of the SITE, not of the town's trade.** Every town has a downwind
  edge and a downstream reach, so the pass sits OUTSIDE the specialisation chain, runs after it, and
  never steals a parcel that chain claimed (`claimed()`): one industry per parcel, first claim wins.
- **`opts.economy` is built whenever either bearing exists**, not only when a specialisation does —
  otherwise an ordinary town carries no vectors at all. Absent, the pass is inert and the UME
  goldens are untouched by construction (v0.98).
- **A district with no tint renders as the default brown** (v1.80/v2.15). All four carry
  `_UM_ECON_TINT` entries and provenance lines, both asserted.
- **A field that is deliberately uncached is still not free to call in a loop.** `currentWindField()`
  measures **36 ms** and this calls it once per settlement — ~8.5 s across 235. Memoised in the
  adapter, never inside `currentWindField` (v1.86 leaves that live on purpose for the debug views),
  on a key naming everything it reads. **36 ms -> 0.01 ms**, with the invalidation asserted.
- **Known scope cuts**: the windmill on the windward rampart is a BUILDING, not a district, and
  belongs to the building grammar; warehouses at the quay were already built (v1.17 S6); mining
  ribbon form is a settlement shape, not a quarter.

### Three scopes, one screen, and seven knobs nothing could reach (v2.63, DCC-line file only)

Owner: *"As with the asset manager id like a window/screen were I can modify settlement parameters,
global per faction but also per individual settlement."* `hash_gen1.js` vs v2.62 **ALL IDENTICAL**;
verification is `tests/perf/probe_settleparams.js` (30 assertions) plus `tests/run.sh` 1280/0,
`run_um.sh` 852/852, `probe_geninfo` 21/0, `probe_assetsbtn` 14/0.

- **Audit the request before building it** (v2.58). Two of the three scopes were already built and
  merely scattered; the third had no control anywhere. So the deliverable is one screen that HOSTS
  what exists plus real plumbing for the one that does not — not three new editors.
- **A second HOST is not a second surface.** `_civPopulatePlaceEditor` and
  `_civPopulateFactionEditor` both already take a `host`, so the new panes call the identical
  functions the map popup and the roster call. v1.57's rule survives by construction, and the probe
  proves it by typing into `_civPeName` from the new pane and reading the change off the place.
- **Not exposing a knob is a decision that has to be written down.** `FARMERS_PER_URBANITE` is
  already per-faction through v1.54's Ag. technology rungs, so a global would be the very defect
  above; `FOOD_BASE_SURPLUS_RATIO` is what `foodSurplusRatio()` pins itself to so existing worlds
  stay bit-identical; `SETTLE_COAST_SWAP_TOLERANCE` is an internal tolerance. Three of ten, refused
  on a constraint rather than on taste (v2.57's rule).
- **ONE list, THREE consumers.** `CIV_PARAM_DEFS` carries each number once — default, live read and
  the Global pane's rows. The four block-2 constants read `civParamDef` now instead of owning their
  own literal. **A hand-written second copy is how v1.72 BUG-A, v2.45 and v2.54 each drifted.**
- **Make the bit-identity STRUCTURAL, not checked.** `state.civParams` starts `{}` and `civParam()`
  falls through to the table, so a world generated without opening the screen *cannot* differ, and a
  pre-v2.63 save carrying no `civParams` block reloads as the world it was (v2.17's convention).
- **Assert the EFFECT, never the storage.** A control that stores a number nothing reads is v2.42's
  defect. Threshold 0.30 -> **94** seeded sites, default -> **67**, 0.60 -> **18**, clear -> 67 again.
- **A ceiling that moves one branch and not the other means two different things.** A first cut
  scaled only `foodSurplusRatio`'s `isDefault` branch, leaving a traditional faction responsive and
  an industrial one deaf. Both read the live value now: R=9 **0.350 -> 0.556**, R=6 **0.450 ->
  0.571**, R=1 **unchanged at 0.750** — and that last one is asserted too, because it is correct
  (the cap is not the binding constraint there) and because asserting it stops a later "make the
  knob always do something" from inventing surplus.
- **v2.46's rule is about the way IN.** The screen is reached from the header and the probe asserts a
  real click LANDS. Once open it is `position:fixed;inset:0` and covers that button, so ✕/Escape are
  the way out — v1.57's existing shell contract, not a split control. What is pinned instead is that
  `aria-pressed` tracks the screen, so the button can never lie about being open.
- **`_civAutoWorld` closes the screen on its own** (`_civRefreshActiveSubPage`'s factions branch
  resets it unconditionally, v1.96/v1.57) — found by the probe timing out on a hidden ✕, not by
  reading. "Re-populate now" therefore closes explicitly, and clicks the real `#civAutoPopulateBtn`
  rather than calling `_civAutoWorld`, so v2.38's busy wrap stays in one place.
- **Disclosed, not fixed**: `probe_placement`/`probe_foodshed`'s shared urbanisation-band assertion
  reads **21.21%** here **and 21.21% on v2.62** under the identical control — pre-existing. The
  settlements list is a flat capped render (`PM_PLACE_MAX=300`) with search reaching the rest;
  virtualising (v1.16's table) is the upgrade path if a world ever needs to browse past it blind.

### A navigable river is a route, and it has a direction (v2.62, DCC-line file only)

Owner: *"these rivers should be navigateable as a route (but with a lower friction/cost) and a course
or direction the water flows (this also informs the travel planner)"*, then *"Now do the navigable
rivers with flow direction."* Civ-layer only — `hash_gen1.js` vs v2.61 **ALL IDENTICAL**; generated
ROAD geometry is a deliberate re-baseline. Verification is `tests/perf/probe_rivernav.js`
(19 assertions, hard-errors on v2.61) plus `tests/run.sh` 1280/0, `run_um.sh` 852/852,
`probe_riverfill` 24/0, `probe_roadconnect` all passed, `probe_travel` every case in band.

- **A per-cell cost cannot tell walking ALONG a river from cutting ACROSS it.** The ford/bridge wait
  was charged on every river CELL, so following a channel priced as fording it once per cell; the
  navigable discount was multiplied into every river cell too, so a road crossing a trunk river
  square-on collected 35% off the very cell it was bridging. Measured against plain ground before the
  fix: order-1 **1.44x**, order-2 **1.71x**, order-3 — the only kind a hull can use — **3.46x**, its
  1.894 h of bridge dwarfing 0.587 h of travel. **The most navigable river on the map was the most
  expensive ground on it.** This is **v2.33's own finding about slope** — *"it is the only place a
  direction exists"* — which that version moved to `edgeCost` and did not carry to the river terms
  sitting ten lines below in the same function. **When you move one term to the edge, grep the rest
  of that function for the others.**
- **Measured after**: ALONG **0.302 h against plain 0.746 (0.41x)**, ACROSS **1.347 h (1.81x)** —
  the whole bridge, still paid — a **4.5x** spread; a river too small to float a hull gets no boat
  whichever way you walk beside it (**0.99x** plain). Real roads follow them: along-river road km
  **251 -> 405 (1.61x)** over three seeds, crossings flat at **0.97x**, total road length slightly
  DOWN — the fix does not buy river mileage by manufacturing bridges.
- **`|align|` is symmetric by construction and that is load-bearing, not incidental.** v1.98:
  an undirected Prim MST has no well-defined answer for an asymmetric cost, and a permanent road
  beside a river is used both ways — its worth cannot depend on which way you are walking today.
  Measured relative asymmetry over 8 000 edge pairs: **exactly 0**. Downstream-versus-upstream is a
  JOURNEY question, answered where a journey has a direction.
- **NAVIGABILITY IS A CATCHMENT AREA, NOT A STRAHLER ORDER, AND THE MEASUREMENT IS WHY.** The three
  existing consumers (`_civPlaceNavigability` v1.35, `buildSettlementSuitability`'s navigable-river
  term, `_civNavigableRiverDiscount` v0.94) all gate on `order>=3` and copying that was the obvious
  move. v2.59 measured what it costs; re-measured here with only the map extent varied, one
  `order>=3` label describes rivers of median catchment **6 417 km² at 800 km against 1 429 009 km²
  at 40 000 km — a 223x swing in what "navigable" would mean**, decided by how wide the map is. The
  catchment is resolution-free too: **483 km² at 512px against 482 at 1024px** on the same world, a
  4x cell-count change.
- **`CIV_RIVER_NAVIGABLE_KM2 = 1000` is anchored twice over, independently** — the head of navigation
  on the Thames at Lechlade and the Severn at Welshpool (both near 1 000 km²), and the discharge rule
  (a small barge wants ~10 m³/s; at a temperate runoff near 300 mm/yr that is ~1 050 km²). It covers
  **30.0% of channel cells at the app default against `order>=3`'s 2.0%** — **42 -> 728** navigable
  routing edges, and one that means the same river on every map.
- **This is a NEW consumer and it moves none of the three above**, so no existing world's
  settlements, water access or road discounts shift. v2.59's nine-consumer migration is still the
  owner's call, untouched here.
- **The river's worth is a SPEED read from the planner's own vessel table, never a constant here.**
  The obvious number is wrong and the measurement says so: a river's advantage is cost per ton, not
  speed — Diocletian's 5.5x road:river ratio (v1.33's own source) — but the router's grid is in HOURS
  and spending a cost ratio as a time ratio is the dimensionless-number defect v2.33 removed from
  this same function. Nor is a river uniformly quicker: through `jpVesselDayKm` the Longship cruises
  **2.18x** walking at 5 000 kg while the River Barge is **0.40x** at 30 000 kg. Both true, different
  questions. `_civRiverTravelSpeed()` prices the fastest hull the water admits, through the planner's
  own eligibility test — **the v2.34 rule applied to water**. Change a vessel's speed and the router
  follows; the probe asserts it. Falls back to 1, so a missing planner can only cost the river its
  bonus, never invent one.
- **The planner's current was a proxy for the water's direction, not the water's direction, and it
  was backwards one step in five.** `_jpRiverCondition` read `(loss-gain)/km` off the route's own
  ELEVATION PROFILE: measured on real roads, **10 of 48** steps got the current backwards. **v1.102
  had already special-cased lakes out of it** — a flat lake bed's DEM noise reads as a real current
  under exactly this proxy — which was this same defect caught once at one symptom. The sign is the
  receiver tree's now and the magnitude the channel's own gradient. **Assert it as a ROUND TRIP**:
  the same 90.6 km chain reads **Strong Downstream** one way and **Strong Upstream** the other,
  which an elevation proxy cannot guarantee and a receiver tree cannot get wrong.
- **`cat:'river'` was structurally unreachable for a real river.** `_jpDeriveStages` classified every
  river cell `cat:'land'` and used its `riv` flag only to count crossings — so the whole vessel /
  `JP_TERRAIN.river` / `JP_ROUTE.river` / `jpVesselDayKm` model applied to lakes and to nothing else.
  A step that both follows a navigable channel and sits on one now derives as river: **54 km of a
  90.6 km chain**, against 0.
- **`_civRiverFlowField()` is ONE definition of which way the water runs**, shared by the router and
  the planner rather than written twice. Keyed on every input it reads (`_fieldGen`, `GW`/`GH`,
  extent, world mode — v2.56), covering **99.8%** of land channel cells; a wrapped receiver is ONE
  step, never a map width (v1.29/v2.37/v2.58, fourth site).
- **Omitting the `river` argument reproduces the v2.61 arithmetic exactly** (v1.98 `edgeCost` /
  v2.40 `opts` discipline), asserted by diffing every non-river edge on a real world; `pts` omitted
  likewise gives `_jpRiverCondition`'s v2.61 answer.
- **Disclosed, not fixed**: the ford's own `fordK` still bands on Strahler order (v0.94), so it
  carries the extent-dependence the gate just shed — changing it re-baselines road geometry for no
  measured reason and is not what was asked. `_civMstRoutes`' sea lanes keep v1.98's edge cost.
  A saved world keeps its roads until they are regenerated.

### A river is painted as water, in the lake's own colour (v2.61, DCC-line file only)
...
function and focus on coloring the river banks accordingly."* A PAINT change plus one classification
fix. Verification is `tests/perf/probe_riverfill.js` (24 assertions) plus `tests/run.sh` 1280/0,
`run_um.sh` 852/852 and the six other river probes.

- **The two water renderers disagreed about what water IS, and that is the whole report.** A LAKE is
  OPAQUE — `lakeWaterColor` RETURNS a colour and the caller writes it, at a flat 0.95 shade, so it
  reads as a level surface. A RIVER was a translucent Beer-Lambert TINT over whatever land colour was
  underneath, at alpha `sV*0.85` where `sV` carries the DISCHARGE MAGNITUDE (`amp` in [0.45,1]) as
  well as the coverage — so a small river was painted at **38% opacity** with the hillshade showing
  straight through it. Measured on the crop the report came from: a flat-edged **2.5 km** slab with
  the terrain's own ridges running visibly through it. A decal, not water.
- **Coverage is a geometric quantity with no free parameter, and it is NOT `sV`.** The river writes
  the lake's colour at how much of the pixel the channel covers: **57.3% of painted cells are fully
  opaque against 0% before.** It is a SEPARATE accumulator, because the widest river over a pixel is
  not necessarily the brightest.
- **Bound the antialias band by the CHANNEL, not by a pixel.** A flat one-pixel edge is right in a
  refined tile and wrong on the coarse grid, where one "tile pixel" IS a cell and `wIn` is 0.8 of
  one: it capped the centreline at 80% and averaged 50% — a tint again, the thing being fixed.
- **`kV`/`wcol` omitted ⇒ v2.60's blend character for character**, so `waterShade` stays reachable
  and the suite's four assertions on it still bite (the v1.98 `edgeCost` / v2.40 `opts` discipline).
  The probe reproduces the old expression rather than trusting the comment.
- **ONE definition of the lake's colour.** It was written out verbatim in THREE — `lakeColor`,
  `lakeColorSampled`, and `renderBiomeTileRGBA`'s inline lake branch — and v2.61 adds a FOURTH
  consumer, so it had to become one function before it could be shared rather than copied again.
- **A depression a river flows into is a LAKE, whatever the local rainfall.** `lakeRain` (0.22) asks
  whether it rains enough HERE to keep a pool — the right question for an unfed hollow (Death Valley,
  Qattara) and the wrong one for a TERMINAL lake, which exists precisely because a river delivers
  water from a wetter catchment. **Chad sits in the Sahel, Eyre and the Aral in deserts, the Dead Sea
  in a hyper-arid basin; every one fails a local-rainfall test and every one is a lake.** The supply
  term was already computed (`flowField`, rainfall-weighted since v2.41) and the threshold is the
  file's own `riverFlowThresh`, so this adds **no constant of its own**. 15 904 -> 17 725 lake cells.
  Optional on the primitive (absent ⇒ v2.60 exactly) and asserted MONOTONE: it can only ever ADD.
- **The sculpt-derived digging pass is REVERTED, and the isolation is exact.** Against v2.59 the
  battery reads `field` **3273059064** / `temp` 2151860328 / `rain` 1311039392 / `flow` 1721724374
  **IDENTICAL on both sides in all five scenarios**, only `rgba` moving. v2.61's terrain IS v2.59's,
  byte for byte — which is what proves step 2c was the whole of v2.60's field divergence.
  `CHANNEL_DESCENT_CENTRE_HALFW` went with it (v2.14's dead-code rule).
- **Remove a stem, never narrow one.** 455 of 1104 stems are isolated, but their median length is 6.2
  cells — real rivers terminating at a pit, and culling them all deletes 41% of the network. What is
  removed is what the geometry condemns: **shorter than it is drawn wide**, self-calibrating, no new
  constant. It is the bound v2.60 looked for and could not use — capping the WIDTH by length broke
  the network **0 -> 111 breaks**, because a thin stub stops covering the elbow to the trunk it
  joins, whereas removing a stem that nothing joins and that joins nothing cannot disconnect anything
  **by construction**. `_stemAttachment` is what makes that a guarantee rather than an assumption.
- **The bank falls out of the walk that is already happening.** The palette existed
  (`applyCoastRiverSDFv`'s damp-bank/wetland/floodplain, v0.097) but was unreachable: gated on
  `sdfRivers` (default 0) and fed by `buildRiverSDF` — **3078 ms** (v2.56) and one of the per-tile
  passes v1.29 lists as a seam because it has a spatial neighbourhood. So the COLOURS are reused and
  the DISTANCE is not. Its width is the river's own, so it inherits whichever floor bound the water.
- **REFUTED and measured, so it is not re-chased**: filling the CARVED trench. At the app default the
  terrain carries no river-scale cross-section — wetted half-width **below the 0.8-cell floor at
  88.1% of vertices** at bed+1 m, p50 **0** — while at the depths where it does, **13-37% saturate**
  into floodplain; at a 50 km region filling to the bank reaches **38-62 cells**, a 3 km flood for a
  headwater. A backwater sweep with no cap floods **650 839 of 670 720 cells**: an unbounded
  fill-to-a-level is a lake-maker, not a river renderer.
- **Disclosed, not fixed**: the carve and the renderer still use two different widths — carve
  0.80/1.30/1.80 cells by order against a drawn 0.80, so **24.6% of the drawn river at the default
  sits in a trench wider than its water, and 100% at a 50 km region, up to 5.5x**. That is a real
  "two functions answering one question" drift; it is left standing rather than resolved by widening
  the paint to a size v2.49 already established is four times the Amazon. Reverting 2c also restores
  v2.59's climb figures (12.55% of drawn steps, 748 on dry slope) — the two depression models still
  disagree and nothing now edits the ground to hide it.
- **A name collision the RUNTIME caught, not a review**: the coverage accumulator was first called
  `k`, which `riverFieldTile`'s own segment loop already uses as its counter, so the allocation
  assigned to the loop variable. Renamed `cov`, with the reason at the declaration.
- **Three probe assertions were RETIRED, not loosened** — they measured v2.60's terrain edit, which
  the owner reverted. The `dry === 0` assertion became "every chain cell of a DRAWN stem", with the
  cull bounded by an **independently recomputed** rule rather than by calling `_stemAttachment` back.
- Also fixed: `<title>`/`#verTag` still read **v2.34**, 26 versions stale (the VERSION drift
  v1.30/v1.52/v1.100 each recorded).

### A river is a continuous feature, not a chain of cells (v2.60, DCC-line file only)

Owner: *"rivers are quickly small strokes one after another and when we zoom in, maybe we should
paint/fill them the same way we do with lakes"*, then *"Be sure a river doesn't break up into parts.
You can also use a bit of the sculpt tools to make sure a length of the river is deep enough to
constitute river."* **A deliberate re-baseline** — `field`, `flow` and `rgba` all move at the app
default. Verification is `tests/perf/probe_riverfill.js` (20 assertions) plus `tests/run.sh` 1280/0,
`run_um.sh` 852/852 and the five river probes.

- **The defect was ONE NUMBER, and its sibling had been right for thirty versions.**
  `buildRiverNetwork` stamps a DISC AT EACH CHANNEL CELL; `halfW` floors at 0.5, so `r=ceil(0.5)=1`
  and only `d===0` passes — one cell per centreline point. A D8 chain steps diagonally **42%** of the
  time and two diagonally adjacent SQUARES touch only at a corner: **4 856 four-connected components
  for 134 rivers**, 2 792 painted cells (21%) with no orthogonal neighbour. `carveRiverValleys` cuts
  the SAME rivers at `halfW=0.8` at order 1 — above a cell's circumradius `sqrt(1/2)=0.7071` — and
  **v2.30 fixed exactly this defect for the carve** ("a diagonal step broke the trench"). Nobody
  carried it to the render stamp, which is the one you can see.
- **8-connectivity is free on a D8 chain and therefore proves nothing.** Every metric here is
  4-connectivity, and the headline one walks every main stem CELL BY CELL asking whether the painted
  water is 4-connected from one chain cell to the next. That is what the eye reads as one line, and
  it is the only measure that cannot be satisfied by painting pixels somewhere else.
- **A floor in TILE PIXELS evaporates exactly where you are looking.** `RIVER_TILE_MIN_PX` is 0.55
  tile px, i.e. **0.014 cells** at a 9.4 km view — so the symbol vanishes as you zoom in, which is
  why a river between two one-cell pit lakes read as a hairline strung between beads.
  `RIVER_MIN_HALF_CELLS=0.8` is the grid's own statement (the centreline is known to +/-half a cell),
  **deliberately in CELLS and not real km** — the eight prior real-km occurrences are about PHYSICAL
  quantities; a resolution floor must bind on a coarse map and release on a fine one, which it does
  for free (0.069 real half-cells at 800 km/1024px, 1.1 at a 50 km region).
- **A symbol is a CROSSOVER, not a curve.** `1-d/w` fades over the DRAWN width, so a bound floor
  spends the whole cross-section on a channel that is not there — 17 081 cells painted, 9 775 reaching
  the shipped amplitude, still 4 365 pieces. Fading over the REAL width was built and also rejected:
  it puts the elbow inside its own fade band, so the BIGGER the river the fainter its corner.
  `rIn>=wIn ? 1-dist/wIn : 1` — the real cross-section where the grid resolves the channel, a FLAT
  mark of one cell's circumradius where it does not. v2.25's `max(symbol,real)` as a profile, and what
  v1.05's lake shoreline already does (a boolean coverage test at full strength, never a fade).
- **A fragment is not a river, and at a 1.6-cell symbol that finally shows.** `traceRiverPolylines`
  emits one polyline per SOURCE: one 14-cell crop held **13 polylines — ONE 59-point stem and TWELVE
  2-3 point stubs**, each drawn wider than it is long. Those are the beans and the scalloped edge.
  v2.58 named this and built `buildMainStems`; the stroked overlay used it, the raster path did not.
  Same crop after: **4**.
- **REFUTED, so they are not re-chased**: capping the floor by the reach's own length (v2.50's "a
  floor that INFLATES is not a bound") removes the beans and takes broken stems **0 -> 111**, because
  a thin stub stops covering the elbow to the trunk it joins — the width that guarantees continuity
  and the width that stops a stub reading as a blob are **the same number**. `rdpSimplify` at eps 0.25
  changes the painted mask by **exactly nothing** (same area, same perimeter, max deviation 0), so the
  scalloping was never the D8 zigzag. A minimum-lake-SIZE gate moves breaks 115 -> 105 — the breaks
  are along flooded reaches of real lakes, not at ponds.
- **The raster must not be cut at lakes, and the file said so itself.** Every raster consumer paints
  the LAKE and not the river on a lake pixel (`renderBiomeTileRGBA` `continue`s, `bakePixel` returns
  `lakeColorSampled`, the main loop overwrites with `lakeColor`), so the cut removed nothing visible
  and opened the junction — all 115 remaining breaks were lake-adjacent and every one diagonal. v1.29
  drew the distinction for the GeoJSON export in these words: *"a lake reach is real hydrology."* The
  STROKED overlay keeps its cut; that is where v1.29's own report lives.
- **The sculpt half is a FINISHING pass, and its size was chosen by measurement.** The carve cuts the
  network built from the PRE-carve field while `_riverNet` is rebuilt from the POST-carve one — so
  **1 433 of 11 414 drawn steps CLIMB** (median 12.7 m, worst 816 m, 748 of them outside any lake).
  Root cause: with `hydro.integrate` on, receivers follow the depression-FILLED surface while
  `buildWaterBodies` runs its own separate priority-flood over the raw field — **two depression models
  answering one question**. This pass does not resolve that; it stops the renderer drawing the
  disagreement. A full re-carve moves **5.16%** of the map for 12.55->9.88%; descent on the chain's
  OWN cells at `CHANNEL_DESCENT_CENTRE_HALFW=0.5` reaches **4.33%** and moves **0.74%**. ONE pass —
  it cannot converge, because every cut moves `flowField` and so moves the network it is chasing.
- **Read the EXTENT, not the stem count** (v2.30's own rule): stems 1 305 -> 1 104 because
  tributaries that used to stop at an uphill step now run on into their trunk, while traced extent
  RISES **13 395 -> 13 907 cells (+3.8%)** and carved coverage goes **87.7 -> 91.6%**.
- **Isolate a re-baseline** (v2.29/v2.50/v2.57/v2.59): `carveRivers` off AND `showRivers` off on both
  sides is **ALL IDENTICAL**; `carveRivers` off alone leaves `field` and `flow` IDENTICAL with only
  `rgba` moving. So the terrain divergence is exactly the finishing pass and the render divergence is
  exactly the river blend.
- **A widened symbol needs a widened REJECT MARGIN.** `riverFieldTile` pads the polyline bbox by the
  maximum REAL half-width while the stamp draws at the floored one, so a polyline outside a tile but
  whose band reached inside was accepted in one tile and rejected in its neighbour — a real seam,
  measured **0.498** on the shared column against v2.40's <0.02 bound. Latent while the radii were
  close; an 11x floor bit immediately.
- **v2.59's own probe still passes 18/18 and its NUMBERS move, which is the re-baseline working.**
  The flip's longest whole main stem reads **66 -> 212 km (3.22x**, against v2.59's published 1.80x),
  the top Strahler bucket spans **145.9x** (123.7x), max order anywhere is **5** (4), and `order>=3`
  covers **0.50 / 4.12 / 4.58%** at 800/8 000/40 000 km (0.32/3.73/4.10). Every conclusion holds —
  order still reports **3 -> 3** across a trunk 3.22x longer and is still non-monotone in 4 of 6.
  **Quote a figure against the version it was measured on.**
- **A probe assertion of my own had to be RETIRED, not loosened.** `probe_riverorder.js` compared
  max Strahler order at 512/1024/2048px on ONE seed and demanded equality; it read 3/3/3 on v2.59 and
  **3/3/4** here, which says which hills seed 12345 grew, not what `riverFlowThresh` does. Over five
  PINNED seeds: v2.59 deepens on **0 of 5**, v2.60 on **1 of 5 by one step**, and v2.59's mean change
  is NEGATIVE — noise both ways, against the **1.7-2.5 levels** Horton's Rb predicts over a 16x cell
  count. The assertion is that aggregate now. **The single-outlier shape v2.25/v2.32 recorded, inside
  an assertion written one version earlier.**
- **`probe_coastgeom.js`'s two failures are NOT this version's, and that was CHECKED.** The identical
  probe against v2.59 with the same control reads **15 passed / 2 failed, the same two**. Its
  bit-identity assertion is a v2.57-only claim by construction (v2.59's `integrate` flip already
  moves `field`, so restoring one blur constant cannot recover v2.56's bytes), and its "cuts the
  bristles by a third" bar was already red on v2.59 by a hair (**14.95%**, a 29.8% cut against the
  33.3% wanted; 15.23% here). **The number describing the SHIPPED state improves**: final bristles
  16.56% -> **16.51%** against v2.56's 21.29%.
- **My own figure surfaced the residual, and it is a `buildMainStems` question, not a width one.**
  Two isolated lozenges sit beside the trunk in the deep-zoom panel: stems that terminate in mid-air,
  **neither at a confluence, nor at sea or a lake, nor at the map edge**. World-wide at seed
  12345/1024px v2.60 cuts every such count — orphan terminations **246 -> 194**, tiny ones **82 -> 58**,
  stems of <=3 points **383 -> 289** — while making each survivor LOUDER (a 2.4-cell headwater is a
  1.6-cell lozenge at the floor instead of a hairline). Removing them is the refuted length-cap, which
  costs 111 broken stems. Disclosed, not fixed; a network change is not bundled into a render report.
- **Disclosed, not fixed**: with Show-lakes OFF a lake pixel renders as land, so the river is then
  painted across the lake bed — its real course, with the lake being what is hidden. The two
  depression models still disagree, which is what leaves 4.33% of steps climbing.
  `probe_lodrivers.js` asserts v2.39's design, which **v2.40** deliberately reverted: 5 of its 13
  have failed on every version since, v2.59 and v2.60 alike.

### Strahler order is saturated; the flip it cannot see (v2.59, DCC-line file only)

Owner, on v2.58's disclosure: *"Turn it on and give me a comparison with for example strahler and if
we should replace it."* **A deliberate re-baseline of every world generated from a seed** — `field`
included. Verification is `tests/perf/probe_riverorder.js` (18 assertions) plus `tests/run.sh`
1280/0, `probe_deltas.js` 18/0 and `probe_riverscale.js` 17/0.

- **A conservative default can be a broken one.** v2.41 measured what leaving `integrate` off costs
  and shipped it off anyway so no world would move. Re-measured inside ONE build at the app's own
  default: **68.5% of land drained into an interior PIT -> 0.0%**, sea-draining land **26 031 ->
  42 475 cells (1.63x)**, longest whole main stem **66 -> 118 km (1.80x)**, biggest catchment
  arriving at a stem mouth **5 078 -> 92 815 km2 (18.3x)**.
- **The state literal and the `loadZip` guard now disagree ON PURPOSE.** A pre-v2.41 save carries no
  `hydro` block and *was* generated without integration, so it must reload as the world it was; a
  v2.41+ save carries its own explicit value. The v2.17 `passes` / v2.22 `crater.physical`
  convention. `deltas` stays OFF — it deposits real sediment, which is a look decision.
- **Isolate a re-baseline or the claim is unfalsifiable** (v2.29/v2.50/v2.51/v2.57). The battery
  diverges on every scenario; with `integrate` forced equal on both sides it is **ALL IDENTICAL BOTH
  WAYS** — v2.58 at `true` produces v2.59's default `field` FNV **3273059064** and v2.59 at `false`
  produces v2.58's **528640695**, exactly. The flip is the whole of it.
- **A re-derived receiver tree is the "two functions answering one question" defect, and it cost a
  probe.** A first cut of the drainage walk built its own D8 tree, omitted the map-EDGE outlet case,
  and read **55% pit** on a world `probe_deltas.js` measures at under 1%. The walk is lifted from
  that probe verbatim now. `currentRoutingSurface()` returns **`null`** when `integrate` is off —
  that is its contract, and the off-state must walk the raw `field`, exactly as `computeFlow` does.
- **REFUTED — Strahler order is NOT resolution-dependent in this engine.** `riverFlowThresh` is
  `gw*gh*0.0004/riverCoarseEase(mapWidthKm)`, keyed on the CELL COUNT, so the channel mask is a
  roughly constant FRACTION of the grid and the ladder does not deepen: **3 / 3 / 3** at
  512/1024/2048px. The classic criticism does not apply here. **And the resolution sweep could not
  have isolated it anyway** — v1.60's `terrainDetailK` makes relief frequency real-km-aware, so
  changing resolution at a fixed extent changes the TERRAIN.
- **It IS extent-dependent, and that is the defect with teeth.** At one seed and one resolution,
...
- **Order cannot rank inside itself** (top bucket spans **123.7x** in real catchment) **and is not
  monotone in catchment** — in **4 of 6** configurations the world's single biggest river is not the
  top-order one, one of them reading order 2 of 4, i.e. below the `order>=3` navigability gate. The
  whole vocabulary reaches **4**, against 8-12 for an Amazon.
- **The flip is the sharpest demonstration of the limit.** Re-routing 68.5% of the world's land and
  lengthening the trunk 1.80x changes the hydrology of every river on the map, and order reports
  **nothing**: max **3 -> 3**, outlet **3 -> 3**. Saturated. **v2.41's own "outlet Strahler 2 -> 3"**
  is true of the network `probe_deltas.js` rebuilds at `riverDensity:1`, not of the shipped
  `_riverNet` every consumer reads — and it fails identically on v2.58, so that drift is pre-existing
  (v2.48's exact EDT, v2.50/v2.51's crater work, v2.57's plate blur). It is replaced by the discharge
  arriving at the coast, which is what the claim was always about.
- **REFUTED — v2.58's stem LENGTH ranks real catchment WORSE than the order it would replace.**
  Mid-ranked Spearman: rho(order) **0.379-0.734** against rho(length) **0.105-0.398**, order winning
  in all six configurations.
- **And that corrects a number I published one version earlier.** v2.58's **0.963** is length against
  `buildMainStems`' own accumulation over `net.recv` — upstream CHANNEL CELLS, not a catchment AREA
  (**0.968-0.983** there, **0.105-0.398** against `computeFlow`'s raster). **v2.41's "two different
  trees", third occurrence, this time inside a figure of mine.** v2.58's fix stands — fragments vs
  whole stems is like-for-like on ONE quantity — but the length gate is a **cartographic disclosure
  rule** (a stem's extent in screen pixels, literally what it measures), never an importance measure.
  `probe_riverscale.js`'s wording was corrected in the same pass.
- **Spearman over a 4-value vocabulary needs MID-RANKS.** Ties dominate; a first cut gave tied values
  arbitrary distinct ranks and read **-0.26 to +0.41** on plainly monotone data. Rank-with-ties, then
  Pearson; the `d^2` shortcut is only valid without ties.
- **RECOMMENDATION — keep Strahler, and stop using it where it has to RANK.** It is a cheap,
  resolution-stable ordinal tier that correlates with real catchment better than anything else in the
  file. What it cannot do is say which of two rivers is bigger (123.7x inside one bucket,
  non-monotone) or carry a threshold that means the same thing on two maps (0.32% vs 4.10%). The
  quantity for those is the catchment area `computeFlow` already accumulates, in km2 — extent-free,
  resolution-free, continuous. That is a change to nine consumers and a re-baseline of every
  generated settlement, so it is **disclosed as the owner's call**, not bundled into a flag flip.
...
- **`buildMainStems` must accumulate on the CHANNEL tree.** A first cut ranked `net.recv` chains by
  ...
  **are two different trees**, exactly as v2.41 recorded. Kahn's algorithm over `net.recv` itself,
  largest tributary kept at each confluence: rho **0.207 -> 0.963**. **v2.59 CORRECTION:** that is
  length against this accumulation — upstream CHANNEL CELLS — not against `computeFlow`'s catchment
  raster, where the same stems measure **0.105-0.398**. The fragments-vs-stems comparison is
  like-for-like on one quantity and stands; reading 0.963 as "length tracks drainage area" does not.
...
- **`state.hydro.integrate` was DEFAULT OFF and load-bearing for this ladder.** Without it 66.5%
  of land drains into an interior pit (v2.41), so stems terminate early: longest **1 799 km** off
  against **4 879 km** on. Flipping it re-baselines every world from every seed, so v2.58 disclosed
  it rather than changing it — an owner decision, **which v2.59 took**.
...
node tests/perf/probe_wardgrain.js A.html [B.html] # v2.67 the WARD sets the plot grain: the table must cover every ward actually PLATTED (derived from par.ward, never a hand-list), the effect must be a per-ward SIGN against that ward's own baseline (mean frontage across wards is not a function of pressure alone — the blocks differ per ward), wardGrain:0 must reproduce B.html EXACTLY, and the depth half must stay INERT (the block waist binds, not the draw — measuring that is what stopped depth being the feature)
node tests/perf/probe_fringe.js A.html [B.html] # v2.68 the agricultural fringe is DRAWN: buildFarmland has pushed field/pasture polys into model.details since v0.95 and NOTHING ever drew one (the City Viewer's detail pass branches on nine kinds and neither is among them) -- so the load-bearing half is the CONTROL, which must generate the identical parcels and draw zero of them; the rest asserts all THREE renderers separately, the ORDER (fringe under water, because it is ground), the hedgerow crossover in BOTH directions (a 5.8 m strip with a dark outline is a dark wash, v2.50 in a renderer), and that the selions already carry the ploughing direction -- which is the measurement the decision not to write a hatch pass rests on
node tests/perf/probe_lodalign.js A.html [B.html] # v2.69 refinement must not move the coastline: reproduce the drift as a share of tile pixels drawn OPPOSITE to the surface settlements are placed on (monotone in LOD depth -- that monotonicity IS the report), decompose it (the filter mismatch, not the noise, and neither obvious suspect runs at the defaults), and assert the absolute afterwards -- refinement drowns ZERO land. The control half is what makes it a measurement: a first cut clamped the wrong one of the TWO places the detail band is added and the probe reported an identical drowned count on both sides
node tests/perf/probe_greenpath.js A.html [B.html] # v2.73 the village green and the footpath class: the plaza kind must be a CROSSOVER in BOTH directions and cross/pond MUTUALLY EXCLUSIVE per town ("a village has a pond" passes on a build that also stands a market cross on the green); the enclosed-alley source must be asserted INERT at the default rules and LIVE at a real deadEndBias, which is the measurement the always-on buildFootpaths source exists because of -- a probe that only counted paths would have reported the feature working while that half was dead; no path length may fall inside a building; and against the CONTROL the LAYOUT must be bit-identical, since hashModel covers graph/blocks/parcels/buildings and not plaza/details/paths. Two of its own assertions were wrong first: _umDrawLayout takes its mScale from lodSpanKm(), the live camera, not from the toScreen it is handed, and the Math.max(1,...) span floor is in that CONSUMER rather than in lodSpanKm -- which is the real GW/1000 px/m ceiling the probe now pins
node tests/perf/probe_riverworld.js A.html [B.html] # v2.72 the 40 000 km river render, two defects: the raster geometry must be CUT where it wraps the antimeridian (assert the MECHANISM -- no segment handed to the drawer may span more than half the map -- which survives any later retune of colour, width or floor), and the render must not spend riverCoarseEase's DETECTION ease on what it DRAWS. Region mode cannot wrap and has ease 1, so BOTH fixes are no-ops there by construction -- which is exactly why every existing river probe was blind to this, and why the CONTROL half is load-bearing: it REQUIRES the previous version to carry 44 wrapped segments. Its own 800 km assertion was wrong first, comparing stems drawn against stems available and so crediting this bar with v2.61's pre-existing length cull
node tests/perf/probe_woodland.js A.html [B.html] # v2.71 nothing the settlement draws may sit on water, and the honest test is a DIFF: render the town, render it again with the settlement layer stripped, and count pixels the town changed INSIDE the water -- a share-of-the-water-body metric reads 0.14% and looks like antialiasing, and a palette match misses an antialiased street edge entirely. It must exercise the REAL path (_civAutoWorld -> _umPlaceContext -> _umModelForNow), because the synthetic fixtures pass either way: buildSite sets waterPoly=[] on the real-water path by design, so the clip has to key on the 22 m mask isWater itself reads. Also asserts woodland is an AREA and never lands on water or on a field
node tests/perf/probe_villagestyle.js A.html # v2.70 the flat village-map palette as a Carto style preset: the load-bearing assertions are that the LOD TILE and the flat BAKE both pick it up (the whole design is that landColorCore/seaColorCore are shared, and a canvas-level tint would pass every other check), and that Default returns the render BYTE-IDENTICAL against this same build's own baseline. Two of its own assertions were wrong first -- reading the canvas before the deferred render() lands, and counting EXACT distinct colours where the claim is about BANDING
node tests/perf/probe_typepreview.js A.html # v2.66 the Types pane — a settlement type previewed from its parameters: the pane must be REACHABLE by a real click, every list must be built from the table that defines it (asserted by COUNT against that table, never a hand-written number), merely RENDERING it must store nothing (or a glance spends the structural bit-identity), and a rule must move the LAYOUT — as an ORDERING between two presets whose rules differ in a known direction, which a no-op wiring cannot satisfy
node tests/perf/probe_industry.js A.html # v2.64/v2.65 the district pass — M-IND: the wind and downstream vectors must be the engine's OWN fields, and the siting must be checkable by EFFECT — the load-bearing assertion is an ORDERING (every tan yard downstream of every mill race in the same town), because a sign test through the market is unsatisfiable on a frontage that sits to one side of it
node tests/perf/probe_settleparams.js A.html # v2.63 the Settlements screen: the header control must be REACHABLE by a real click (v2.46), the seven knobs must default to the constants they replaced (so bit-identity is structural), and a knob must measurably change what Auto-populate produces — a control that stores a number nothing reads is v2.42's defect
node tests/perf/probe_rivernav.js A.html # v2.62 a navigable river is a ROUTE and it has a DIRECTION: along-vs-across on the SAME water (a per-cell cost cannot tell them apart), the edge cost EXACTLY symmetric as an undirected Prim MST requires, the gate keyed on catchment km2 rather than Strahler order (with the two measurements that justify it), and the planner's current asserted as a ROUND TRIP — the same chain both ways must read opposite
node tests/perf/probe_riverfill.js A.html # v2.61 a river is painted as WATER in the lake's own colour, at true coverage, with banks; a river-fed pit is a LAKE; the sculpt-derived digging pass is gone; and (v2.60) a river must not break into parts — walks every main stem cell by cell asking whether the painted water is 4-CONNECTED (8-connectivity is free on a D8 chain and passes on the broken build)
node tests/perf/probe_riverorder.js A.html # v2.59 integrated drainage as the default, and Strahler order measured against catchment area (it is saturated, extent-dependent and non-monotone; stem length is worse still)
