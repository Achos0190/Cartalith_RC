...
`Cartalith Gen1 v*.html`** (currently **v0.88**) — a zero-dependency HTML/JS/CSS application,
designed to open via `file://` (a local HTTP server is an accepted fallback for Workers/WASM
threads; `file://` must degrade gracefully, never break).

| File | Role |
|------|------|
| `Cartalith Gen1 v0.88.html` | **Current** unified tool (~15.7k lines, 3 script blocks — see architecture below) |
| `Cartalith Gen1 v0.57/v0.6/v0.61…v0.87.html` | Previous Gen1 versions (kept; never edit in place) |
...
| `tests/` | Headless verification harness (`run.sh`, stubs, 917-assertion suite) + `tests/perf/` Playwright A/B + UI-smoke harnesses |
...
- **After any change to the engine (script block 1): run `tests/run.sh`.** A change is not done
  until it passes (917 assertions green).
...
```bash
tests/run.sh                        # newest Gen1 file: extract engine → node --check → 917-assertion suite
