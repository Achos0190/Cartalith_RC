...
**P1** rows for sea currents and wind: v1.97 made travel *time* directional, this makes the *lane
itself* prefer genuinely faster water. Hash vs v1.97 **ALL IDENTICAL** — sea lanes are civ-layer,
generated by Auto-World/Generate-Roads, never by `generate()`/`renderNow()`.

- **U4 — `edgeCost` hook in `roadDijkstra`.** Every cost model in this file up to v1.97 was
  `cost(cell)`: a scalar field. But currents, wind and river flow are all `cost(from → to)` — they
  help one way and hinder the other. The relaxation loop already evaluated a per-edge step
  (`0.5*(cost[i]+cost[j])`), so it was one seam from supporting that. `edgeCost` is an optional
  trailing `(i,j,dx,dy) => cost` callback; **omitting it takes the identical arithmetic path**, so
  every pre-v1.98 call site is bit-identical by construction (asserted directly by comparing the
  full `dist` array with and without a null callback).
- **U5 — `_civSeaTimeEdgeCost`.** The sea-lane cost grid was the literal constant `1` per ocean
  cell, so lane geometry was pure shortest-distance-over-water and the v1.77–v1.82 current/wind
  vector fields had no influence on it whatsoever. Now each edge is priced by sailing time: rig
  polar (`jpSailFactor`, v1.97) against the local true wind angle, plus the along-track current
  component. The coarse fields are resampled onto the routing grid **once** per build — they are
  deliberately uncached (v1.86), so a per-edge fetch would rebuild the entire wind+current solution.
- **Why ROUND-TRIP, and why that is not a fudge.** A Prim MST is **undirected** — you cannot build
  a spanning tree over an asymmetric cost without inventing a tie-break. Rather than pretend to
  solve asymmetric-MST, each edge is costed at the **mean of its two directional times**. That is
  the correct objective for the thing actually being built: a permanent trade lane is sailed *both
  ways*, so the round trip is what matters. It is symmetric by construction, so the MST stays
  well-defined and the result stays deterministic (asserted).
  It is **not** a no-op that averages the current away, because the polar is non-linear: a lane
  along the wind axis is a fast broad reach one way and a beat the other, so its round trip is worse
  than a cross-wind lane that is a moderate reach in both directions. Historical trade routes
  organised around the winds for exactly this reason.
- **`_CIV_LANE_TACK_FLOOR=0.25`.** A polar value of 0 means "cannot sail this angle directly", not
  "cannot get there" — a square-rigger tacks, making ground to windward slowly and over a longer
  sailed track. Without the floor an upwind edge would cost Infinity, which would read as
  impassable ocean and break the MST.
- **Evidence — and a first measurement I declined to report as a win.** The obvious aggregate test
  (mean sailing quality across whole generated worlds' lanes) moved only +0.35%, which is noise;
  that turned out to be a sample-size artefact — a world has only ~1–6 sea lanes and their endpoints
  are fixed ports, so there is very little for the router to choose between. The controlled test is
  the real evidence: on 4 seeds × ocean pairs, comparing the uniform-distance path against the
  time-costed path **on the same water under the same metric** — **36 better, 0 worse, 1 tie**, mean
  time gain 1.4–6.5%, max **15.3%**, with the router accepting paths up to **24% longer** to get it.
  That is the audit's Test D ("a longer route may be faster") passing on real terrain.
- **Reference rig is square** (`_CIV_LANE_REF_VESSEL='Cog'`): a lane is shared infrastructure, not
  one vessel's voyage, so a fixed reference is the honest choice, and the more wind-constrained rig
  makes lanes avoid persistently unsailable axes.
- **Known scope cuts**: lane geometry is a deliberate, disclosed re-baseline — a regenerated world's
  sea lanes will differ from v1.97's (verified they move on every seed tested). Land routing is
  untouched (`edgeCost` is null for `isSea=false`). River-mode geometry still has no dedicated
  domain — `'mixed'` prefers rivers, it cannot require them (pre-existing, documented at
  `_jpModeForRoute`). Seasonality, storm/wave risk and multi-objective routing remain P3 and
  unbuilt; the audit's §L still advises against the six optimisation modes.
