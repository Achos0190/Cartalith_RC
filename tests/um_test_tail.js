/* Urban Morphology engine — headless assertion suite (v0.95 port of
 * urban-morphology/tests/test_tail.js, adapted for the UME block embedded in the newest
 * Cartalith Gen1 v*.html: `generate` was renamed `cityGen` at the export boundary — see
 * tests/run_um.sh, which extracts the UM-ENGINE-START/END block and passes it in UM_ENGINE).
 ...
 * and statistical acceptance against the register bands (urban-morphology/docs/03-mathematical-assumptions.md).
 */
'use strict';
...
for (const seed of SEEDS) {
  ...
  if (seed === 12345) {
    const mB = UME.cityGen(54321, GENOPTS);
...
{
  const m = UME.cityGen(12345, GENOPTS);
...
for (const kind of ['bay', 'coast']) {
  const tag = `[${kind}] `;
  const m = UME.cityGen(31337, { epochs: 8, pop: 4000, site: kind });
  const m2 = UME.cityGen(31337, { epochs: 8, pop: 4000, site: kind });
...
/* ---------- walls follow the water; optional bastioned star fort ---------- */
{
  ...
  const m = UME.cityGen(12345, { epochs: 8, pop: 5000, walls: true });
  const w = m.wall;
  ...
  const hc = UME.cityGen(31337, { epochs: 8, pop: 5000, site: 'coast', walls: true });
  ...
  const f = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, fortified: true });
  ...
  const f2 = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, fortified: true });
  ...
  {
    const cw = UME.cityGen(7, { epochs: 8, pop: 5000, walls: true });
  ...
  const small = UME.cityGen(12345, { epochs: 8, pop: 2400, walls: true, fortified: true });
  ...
  const nw = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: false, fortified: true });
  ok(!nw.fortified && !nw.wall.ring, 'no fort without a wall');
}

/* ---------- landlocked site (no water at all) ---------- */
{
  const m = UME.cityGen(12345, { epochs: 8, pop: 5000, walls: true, site: 'landlocked' });
  ...
  const lf = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, fortified: true, site: 'landlocked' });
  ok(lf.wall.style === 'bastioned' && lf.wall.fort.wetDitch === false, 'landlocked fort has a DRY ditch (no water to draw)');
  const m2 = UME.cityGen(12345, { epochs: 8, pop: 5000, walls: true, site: 'landlocked' });
  ok(UME.hashModel(m) === UME.hashModel(m2), 'landlocked deterministic');
}

/* ---------- religious building optional; hamlets have none ---------- */
{
  const none = UME.cityGen(7, { epochs: 8, pop: 5000, walls: true, faith: 'none' });
  ...
  const hamlet = UME.cityGen(7, { epochs: 6, pop: 450, walls: false });
...
for (const site of ['river', 'landlocked']) {
  const m = UME.cityGen(21, { epochs: 8, pop: 7000, walls: true, fortified: true, site });
  const tag = `[${site}] `;
...
{
  const m = UME.cityGen(31337, { epochs: 8, pop: 6000, site: 'coast', walls: true, faith: 'church' });
...
/* ---------- selectable worship rite + civic hall style ---------- */
{
  for (const [faith, name] of [['church', 'Church'], ['temple', 'Temple'], ['shrine', 'Shrine'], ['mosque', 'Mosque']]) {
    const m = UME.cityGen(12345, { epochs: 8, pop: 5000, walls: true, faith });
    ...
    const m2 = UME.cityGen(12345, { epochs: 8, pop: 5000, walls: true, faith });
  ...
  const bas = UME.cityGen(5, { epochs: 8, pop: 6000, walls: true, civicStyle: 'basilica' });
  ok(bas.civic && bas.civic.style === 'basilica' && bas.civic.apse && bas.civic.apse.length >= 3, 'civic basilica has an apse');
  const log = UME.cityGen(5, { epochs: 8, pop: 6000, walls: true, civicStyle: 'loggia' });
  ok(log.civic && log.civic.style === 'loggia' && log.civic.columns.length >= 3 && !log.civic.belfry, 'civic loggia is a colonnade, no belfry');
  const th = UME.cityGen(5, { epochs: 8, pop: 6000, walls: true, civicStyle: 'townhall' });
  ok(th.civic && th.civic.belfry, 'town hall has a belfry');
  const mq = UME.cityGen(5, { epochs: 8, pop: 6000, walls: true, faith: 'mosque', civicStyle: 'auto' });
  ok(!mq.civic, 'mosque rite (auto civic) → no monumental town hall');
  const tmp = UME.cityGen(5, { epochs: 8, pop: 6000, walls: true, faith: 'temple', civicStyle: 'auto' });
  ok(tmp.civic && tmp.civic.style === 'basilica', 'temple rite (auto civic) → basilica');
}

/* ---------- river that runs through the town (both banks) ---------- */
{
  const m = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, site: 'riverthrough' });
  ...
  const m2 = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, site: 'riverthrough' });
...
/* ---------- selectable harbour protection ---------- */
{
  for (const [k, t] of [['chain', 'chain'], ['seawall', 'seawall'], ['molefort', 'molefort']]) {
    const m = UME.cityGen(999, { epochs: 6, pop: 4000, site: 'bay', harbourDefence: k });
    ok(m.harbour && m.harbour.defence && m.harbour.defence.type === t, `harbour defence '${k}'`);
  }
  const none = UME.cityGen(999, { epochs: 6, pop: 4000, site: 'bay', harbourDefence: 'none' });
  ok(none.harbour && !none.harbour.defence, "harbour defence 'none' leaves it unprotected");
  ok(UME.cityGen(999, { epochs: 6, pop: 4000, site: 'coast', harbourDefence: 'auto' }).harbour.defence.type === 'molefort', 'open coast auto → mole-head fort');
  ok(UME.cityGen(999, { epochs: 6, pop: 4000, site: 'river', harbourDefence: 'auto' }).harbour.defence.type === 'chain', 'river port auto → chain & towers');
}

/* ---------- amenities scale with settlement rank (village → city) ---------- */
{
  const village = UME.cityGen(5, { epochs: 6, pop: 900, walls: false });
  const town = UME.cityGen(5, { epochs: 8, pop: 5000, walls: true });
  const city = UME.cityGen(5, { epochs: 9, pop: 16000, walls: true });
...
/* ---------- wet-moat feasibility + no overlap into water or fortifications ---------- */
{
  ...
  ok(UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, fortified: true }).wall.fort.wetDitch,
    'fort by the water gets a wet ditch');
  for (const site of ['river', 'riverthrough', 'bay', 'coast']) {
    const m = UME.cityGen(21, { epochs: 8, pop: 5000, walls: true, fortified: true, site });
    const tag = `[${site}] `;
...
{
  const small = UME.cityGen(777, { epochs: 8, pop: 1200 });
  const large = UME.cityGen(777, { epochs: 8, pop: 12000 });
  ...
  const noWall = UME.cityGen(777, { epochs: 8, pop: 5000, walls: false });
  ...
  const noWall2 = UME.cityGen(777, { epochs: 8, pop: 5000, walls: false });
  ...
  for (const culture of profiles) {
    for (const site of sites) {
      for (const fortified of [false, true]) {
        for (const seed of seeds) {
        const m = UME.cityGen(seed, { epochs: 8, pop: 7500, walls: true, fortified, culture, site });
        checked++;
        if (m.wall.ring) {
...
/* ---------- Phase 1 architecture: culture profiles (docs/07) ---------- */
{
  ...
  const base = UME.cityGen(12345, { epochs: 8, pop: 5000 });
  const explicit = UME.cityGen(12345, { epochs: 8, pop: 5000, culture: 'medieval' });
  const fallback = UME.cityGen(12345, { epochs: 8, pop: 5000, culture: 'bogus' });
...
{
  const r1 = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, culture: 'medieval', ruined: true });
  const r2 = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, culture: 'medieval', ruined: true });
  ...
  for (const culture of ['medieval', 'venus']) {
    const m = UME.cityGen(12345, { epochs: 8, pop: 6000, culture });
  ...
  for (const culture of ['medieval', 'venus']) {
    const off = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, culture });
    const on = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, culture, ruined: true });
  ...
  const ruinedMedievalFort = UME.cityGen(12345, { epochs: 8, pop: 8000, walls: true, fortified: true, culture: 'medieval', ruined: true });
  ...
  const ruinedVenus = UME.cityGen(12345, { epochs: 8, pop: 8000, walls: false, culture: 'venus', ruined: true });
  ...
  for (const site of ['river', 'riverthrough', 'bay', 'coast', 'landlocked']) {
    const p1 = UME.cityGen(2024, { epochs: 8, pop: 6500, walls: true, culture: 'venus', ruined: true, site });
    const p2 = UME.cityGen(2024, { epochs: 8, pop: 6500, walls: true, culture: 'venus', ruined: true, site });
...
{
  const g1 = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, culture: 'medieval' });
  const g2 = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, culture: 'medieval' });
  ...
  const small = UME.cityGen(12345, { epochs: 8, pop: 1200, walls: true, culture: 'medieval' });
  ...
  const m = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, culture: 'medieval' });
  ...
  for (const site of ['river', 'bay', 'landlocked']) {
    for (const seed of [12345, 2024, 777]) {
      const mm = UME.cityGen(seed, { epochs: 8, pop: 7000, walls: true, culture: 'medieval', site });
  ...
  const ven = UME.cityGen(12345, { epochs: 8, pop: 9000, walls: false, culture: 'venus' });
  ...
  for (const site of ['river', 'riverthrough', 'bay', 'coast', 'landlocked']) {
    for (const seed of [12345, 2024, 777]) {
      const mm = UME.cityGen(seed, { epochs: 8, pop: 9000, walls: true, fortified: true, culture: 'medieval', site });
      checked++;
  ...
  for (const site of ['river', 'riverthrough', 'bay', 'coast', 'landlocked']) {
    const s1 = UME.cityGen(2024, { epochs: 8, pop: 6500, walls: true, culture: 'medieval', site });
    const s2 = UME.cityGen(2024, { epochs: 8, pop: 6500, walls: true, culture: 'medieval', site });
...
{
  const ve = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: false, culture: 'venus' });
  const ve2 = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: false, culture: 'venus' });
  ...
  const veWalled = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, culture: 'venus' });
  ok(!!veWalled.wall.ring, 'venus can be walled on request — walls are a real optional toggle, not forced off');
  const vef = UME.cityGen(12345, { epochs: 8, pop: 8000, walls: true, fortified: true, culture: 'venus' });
  ...
  const vefLandlocked = UME.cityGen(12345, { epochs: 8, pop: 8000, walls: true, fortified: true, culture: 'venus', site: 'landlocked' });
  ...
  for (const site of ['river', 'riverthrough', 'bay', 'coast', 'landlocked']) {
    const v1 = UME.cityGen(2024, { epochs: 8, pop: 6500, walls: true, culture: 'venus', site });
    const v2 = UME.cityGen(2024, { epochs: 8, pop: 6500, walls: true, culture: 'venus', site });
...
/* ---------- GenerationRules system (docs/07 §3.4) ---------- */
{
  ...
  const noRules = UME.cityGen(12345, { epochs: 8, pop: 5000 });
  const explicitDefaultRules = UME.cityGen(12345, { epochs: 8, pop: 5000, rules: UME.resolveRules() });
  ...
  const wildModel1 = UME.cityGen(12345, { epochs: 8, pop: 5000, rules: wild });
  const wildModel2 = UME.cityGen(12345, { epochs: 8, pop: 5000, rules: wild });
 ...
 * is the separate, default-off switch that lets it actually exclude parcels from building. */
{
  ...
  for (const culture of ['medieval', 'venus']) {
    for (const site of ['river', 'bay', 'landlocked']) {
      const m = UME.cityGen(4242, { epochs: 8, pop: 6000, culture, site });
      for (const p of m.parcels) {
        checked++;
  ...
  const noOpt = UME.cityGen(12345, { epochs: 8, pop: 5000 });
  const explicitFalse = UME.cityGen(12345, { epochs: 8, pop: 5000, terrainAware: false });
  ...
  for (const seed of [1, 2, 3, 4242, 777, 12345, 999, 55, 88]) {
    const river = UME.cityGen(seed, { epochs: 8, pop: 7000, site: 'river' });
  ...
  const ta1 = UME.cityGen(12345, { epochs: 8, pop: 7000, terrainAware: true, site: 'coast' });
  const ta2 = UME.cityGen(12345, { epochs: 8, pop: 7000, terrainAware: true, site: 'coast' });
  ...
  for (const seed of [1, 4242, 777, 12345, 999]) {
    for (const site of ['river', 'riverthrough', 'bay', 'coast', 'landlocked']) {
      const off = UME.cityGen(seed, { epochs: 8, pop: 7000, site });
      const on = UME.cityGen(seed, { epochs: 8, pop: 7000, site, terrainAware: true });
  ...
  for (const seed of [4242, 777, 12345, 999]) {
    for (const site of ['river', 'riverthrough', 'bay', 'coast']) {
      const m = UME.cityGen(seed, { epochs: 8, pop: 7500, walls: true, terrainAware: true, site });
      checkedTA++;
 ...
 * the post-launch simplification pass (docs/07 §3.10). */
{
  ...
  const f1 = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, culture: 'medieval', site: 'river' });
  const f2 = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, culture: 'medieval', site: 'river' });
  ...
  for (const culture of ['medieval', 'venus']) {
    const m = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, culture, site: 'river' });
  ...
  const med = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, culture: 'medieval', site: 'river' });
  ...
  const ven = UME.cityGen(12345, { epochs: 8, pop: 6000, walls: true, culture: 'venus', site: 'river' });
  ...
  for (const culture of ['medieval', 'venus']) {
    ...
    for (const seed of [1, 2, 3, 12345, 999]) {
      const m = UME.cityGen(seed, { epochs: 8, pop: 6000, walls: true, culture, site: 'river' });
  ...
  for (const culture of ['medieval', 'venus']) {
    for (const site of ['river', 'riverthrough', 'bay', 'coast', 'landlocked']) {
      for (const seed of [12345, 2024, 777]) {
        const m = UME.cityGen(seed, { epochs: 8, pop: 7000, walls: true, fortified: true, culture, site });
        auditChecked++;
 ...
 * between generations and that a young settlement's own lifespan can't afford a second circuit. */
{
  ...
  const m = UME.cityGen(12345, { epochs: 10, pop: 9000, walls: true, wallGenerations: true, settlementAge: 600, culture: 'medieval', site: 'river' });
  ...
  for (const seed of [1, 5, 7, 12345, 999]) {
    const young = UME.cityGen(seed, { epochs: 8, pop: 9000, walls: true, wallGenerations: true, settlementAge: 50, culture: 'medieval', site: 'river' });
  ...
  const m2 = UME.cityGen(12345, { epochs: 10, pop: 9000, walls: true, wallGenerations: true, settlementAge: 600, culture: 'medieval', site: 'river' });
  ...
  for (const seed of [1, 5, 7, 21, 42, 100, 777, 999, 31337, 12345]) {
    for (const site of ['river', 'bay', 'coast']) {
      const mm = UME.cityGen(seed, { epochs: 10, pop: 9000, walls: true, wallGenerations: true, settlementAge: 600, culture: 'medieval', site });
      capChecked++;
  ...
  for (const seed of [1, 42, 777, 12345]) {
    for (const site of ['river', 'bay', 'landlocked']) {
      ...
      const omitted = UME.cityGen(seed, opts);
      const explicitFalse = UME.cityGen(seed, Object.assign({}, opts, { wallGenerations: false }));
      neutralChecked++;
  ...
  for (const seed of [1, 42, 777]) {
    ...
    const off = UME.cityGen(seed, opts);
    const on = UME.cityGen(seed, Object.assign({}, opts, { wallGenerations: true, settlementAge: 600 }));
  ...
  for (const seed of [1, 4242, 777, 12345, 999]) {
    for (const site of ['river', 'riverthrough', 'bay', 'coast']) {
      const mm = UME.cityGen(seed, { epochs: 8, pop: 9000, walls: true, wallGenerations: true, culture: 'medieval', site });
      auditChecked++;
  ...
  const capped = UME.cityGen(12345, { epochs: 10, pop: 9000, walls: true, wallGenerations: true, settlementAge: 600, culture: 'medieval', site: 'river', rules: customSettlement });
  ...
  const noCarryModel = UME.cityGen(12345, { epochs: 10, pop: 9000, walls: true, wallGenerations: true, settlementAge: 600, culture: 'medieval', site: 'river', rules: rulesNoCarry });
