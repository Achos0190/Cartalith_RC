...
Open the newest **`Cartalith Gen1 v*.html`** (currently **`Cartalith Gen1 v1.44.html`**) in a
...
| Path | What it is |
|------|------------|
| `Cartalith Gen1 v1.44.html` | **The tool.** Current version |
| `Cartalith Gen1 v0.57/v0.6/v0.61…v1.29.html` | Previous versions (kept, frozen) |
...
| `fractal-geology/` | Standalone stamp-based terrain-sculpt PoC; its engine was ported into Gen1's Generate → Sculpt sub-tab (v1.15) — kept as reference, never edited |
...
```bash
tests/run.sh                                 # newest Gen1 file → 1001-assertion headless suite
