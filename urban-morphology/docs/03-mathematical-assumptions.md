...
| M-GRW-2 | Growth episodes | population logistic; wall circuits built when built-up ≥ ~80% of enclosed area; successive circuits (1–3 typical for towns that persist) | – | walled-city histories (Florence, Cologne circuits; Kostof 1992) | M | Quantized growth: episodes visible as plan-unit seams — a key "grown" tell. Implemented via the opt-in `wallGenerations` toggle (default off; docs/07 §3.11): see M-GRW-2a (fill/spillover trigger), M-GRW-2b (growth pacing/carrying-capacity), and M-GRW-2c (settlementAge real-year gating) for the specific mechanisms. |
| M-GRW-2a | Wall-generation trigger: interior occupancy + extramural spillover | `wallOccupancy(g,ring)` measures two things separately, both required: (1) `fillFraction` — the convex-hull area of degree≥2 nodes **strictly inside** the active ring, divided by the ring's own area (naturally bounded well under 1, since interior nodes can never exceed the ring's own footprint); crossing `wallGenerationThreshold` (default 0.8) means the ~10%+16m growth reserve the wall was built with is now mostly used up; (2) exterior (ribbon-suburb) node count reaching `wallGenerationExtramuralShare` (default 0.15) of the interior count, floor 10 — growth must have genuinely spilled past the wall too, not just filled it. Only once both hold (and the age gate, M-GRW-2c) does the wall supersede: old ring → `ringroad`, new bigger ring raised in its place, up to `maxWallGenerations` (default 3) | ratio | operationalizes M-GRW-2's "≥~80% of enclosed area" and the historical faubourg-then-incorporation pattern (cities filled interior plots and spilled into suburbs along roads, which were "eventually brought together within new, larger city walls") directly, rather than a proxy that could be satisfied by either alone | M (the 80% citation, and the qualitative fill-then-spill ordering) / L (the specific 0.15 extramural-share fraction is a PoC convention, tuned not measured — same honesty flag as M-FOR-4's threshold) | **Superseded an earlier, broken version of this same row, found by direct epoch-by-epoch instrumentation after a user review reported ring roads "generating in advance of the growth":** the first cut compared a freshly recomputed hull of ALL built nodes (already including any extramural growth) against the previous wall's own area — but since the wall itself IS that same hull construction inflated by the SAME ~10%+16m growth reserve, any new growth at all (even a couple of stray ribbon-suburb nodes) made a fresh hull exceed the old wall's area almost immediately; instrumented trace showed generation 2 firing the very epoch after generation 1 was built (ratio 1.106 with only 82 exterior nodes), and generation 3 the epoch after that — practically every epoch, not "1-3 over a town's life." The interior-only fillFraction is bounded by construction (can't repeat that failure mode), and the separate extramural-share requirement makes "spilled past the wall" an explicit, independently-checked condition rather than a side effect of the same buggy ratio. |
| M-GRW-2c | Successive-circuit real-world timing (`settlementAge` input) | a new user-facing "Settlement age (years)" input (default 300, range 30–1000) spreads this run's epochs evenly across real years (`yearsPerEpoch = settlementAge/epochs`); a wall generation is only eligible to be superseded once `wallGenerationMinAgeGap` (default 120 years) worth of epochs have passed since it was built (`(ep − wallState.epoch) ≥ wallGenerationMinAgeGap/yearsPerEpoch`) — so a young settlement's entire epoch budget simply cannot afford a second circuit (verified: a 50-year-old settlement never supersedes its first wall across 5 seeds, however dense its interior gets), while an old settlement's epochs are each worth many years and several circuits fit naturally | years | real successive-circuit gaps: **Cologne** 1106 → 1179/1180 expansion, ~73–74y; **Florence** Matildine (1078) → First Communal (1172–75), ~94–97y, then First Communal → Second Communal (started 1285), ~110–112y; **Paris** Philip II (built ~1190–1213) → Charles V (1358–1371), ~145–168y | H (the individual circuit dates themselves, each independently documented) / L (120y as a single PoC point-value chosen inside that real ~74–168y band, and the linear years-per-epoch spread, tuned not measured) | This is the piece the user explicitly asked for ("an input for the settlement age... population/population growth go hand in hand... ties in to local resources and carrying capacity"): age is now a real, user-controlled input, not an implicit side effect of epoch count, and it is what actually enforces "building up to and beyond the wall before creating a new wall" by making the ordering a genuine function of elapsed time, not just instantaneous geometry. |
